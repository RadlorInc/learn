...
|---|---|---|---|
| **1** | **No attorney-approved privacy policy, terms, or COPPA notice.** ⚠️ **Row corrected 2026-08-17 — it said "no `/legal` route exists at all", which is no longer true.** `src/app/legal/[slug]` and `content.ts` ship, and the pages render with JS disabled. What is open is the *text*: `DRAFT = true` is live on production, and a gate asserts `DRAFT === false` before this can be called done. | `src/app/legal/content.ts` | Marketing a children's product in the US without these is the one failure that is not just embarrassing but *legal*. Long lead time — an attorney takes weeks. **Now a paste, not a build.** |
...
| **3** ⚠️ **half-fixed** | **No durable error retention.** Row corrected 2026-08-17: crashes were never going nowhere — both paths `console.error` into Vercel logs, and `src/infra/errorSink.ts` is now the single sink for the browser boundary *and* `onRequestError`. What is missing is **retention**: Hobby keeps runtime logs ~1 hour, and the `error_events` table (durable, 90-day `pg_cron` TTL) stays empty until `SUPABASE_SERVICE_ROLE_KEY` is set — the same key that gates V13. | `src/infra/errorSink.ts`, `src/instrumentation.ts` | A 2am crash is gone by breakfast. That is exactly the condition the plan-pointer P0 survived three months in. **One env var closes this and V13 together.** |
...
| ~~5~~ ✅ | ~~**No product analytics at all.** Zero analytics deps installed.~~ **DONE — the row was stale.** Self-hosted analytics ships in `src/infra/analytics.ts` → `learner_events`. Measured on prod 2026-08-17: **1,191 `session_start`, 797 `chapter_open`**, last event today. No third-party dep, which is *better* for a children's product — no data leaves the estate. | `src/infra/analytics.ts`; `select event, count(*) from learner_events` | ⚠️ Still worth watching: **`practice_complete` has 0 rows.** No child has played since the plan-pointer P0 fix deployed (last `chapter_open` 2026-08-15), so this is "no data yet" rather than "still broken" — but it is the event that proves that fix works. |
...
| **9** ⚠️ **half-fixed** | **`diagnostic_leads` still accepts anonymous INSERT** (V13). The app-level limit now exists (`/api/lead`, 6/min + email shape) and the RLS policy enforces a real email shape — but the anon `INSERT` **grant** is still there, so anyone holding the public anon key (it ships in the bundle by design) can POST `/rest/v1/diagnostic_leads` directly and skip both. Proven exploitable 2026-08-17. | `V13` in [security.md](security.md); grant confirmed live | Spam and cost on the public funnel; **not** exfiltration (anon has no SELECT — verified). **The close is blocked on F-tier:** set `SUPABASE_SERVICE_ROLE_KEY` in Vercel → apply `20260816170000_leads_server_only.sql` → submit one real lead and confirm it lands. Doing those out of order kills lead capture, because `/api/lead` falls back to the anon key. |
...
| **B12** | 🔴 **There is NO backup of the children's data, and no point-in-time recovery.** Supabase **free plan has neither**. `backup.yml` reports green and writes **zero bytes** — three unset secrets — so the dashboard now *claims* a backup exists, which is worse than the honest nothing it replaced. Fix = **Supabase Pro, $25/month** (daily backups + 7-day PITR), already in the cost model and simply not turned on. | **[F]** turns Pro on, **[C]** re-points `backup.yml` and proves a restore | Founder's call, 2026-08-24: *we cannot take a parent's money for a service whose entire record of their child's progress has no recovery path.* ⚠️ `supabase/schema/baseline_schema.sql` gives the STRUCTURE back and **none of the data** — do not read it as a backup. |
