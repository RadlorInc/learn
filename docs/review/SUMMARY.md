...
| BUG-01 | progress sync / offline | High | Reproduced | M | Queued answers/points dropped when flushed by another account or no session (42501 → drop) | fix now | Fix (planned) |
| BUG-02 | progress sync / DB | High | Reproduced | M | Stale device rolls back level/mastered on the account; level_up re-minted | fix now | Fix (planned) (last-answered wins, no demotion policy change) |
| BUG-03 | consent | High | Reproduced | S | Consent grant failing after B3 scheduled: orphan "you gave permission" email + 24 h Resend-409 lockout | fix now | Fix (planned) |
| BUG-07 | auth | High | Reproduced | S | getMyRole returns null on a read error → wrong redirect / RolePicker can overwrite a real role | fix now | Fix (planned) |
| BUG-09 | consent / retention | High | Reproduced | S | Consent granted on an unconfirmed address; nightly prune cascade-deletes the granted consent record | fix now | Fix (planned) (prune guard); flow → Needs Rafi N5 |
...
| FND-11 | ops/COPPA | High | Measured | S | Consent is email-dependent and has failed silently once (Resend 401 on B3 cancel); quotas unmeasured | fix now | Fix (planned) (B3 cancel retry, with OPS-04) |
...
| MAP-04 | privacy / child | High | Suspected | S | The child's first name is spoken through `speechSynthesis`; with no local English voice the code picks Chrome's network "Google US English" voice, which sends the text to Google — a third party not in doc 07 | fix now | Fix (planned) |
...
| OPS-02 | deploy | High | Reproduced | S | `migrations-changed` diffs only the current push → a rejected/failed/cancelled-pending migration is never retried while app deploys continue | fix now | Fix (planned) |
| OPS-04 | monitoring | High | Measured | M | Nobody is paged: crashes, outages, backup failures reach no person (backup red 13 nights unnoticed; 1 h Vercel logs; GitHub cron 5 h late) | fix now | Fix (planned) (daily numbers-only digest); uptime vendor → Needs Rafi N7 |
| OPS-07 | monitoring / consent | High | Suspected | S | pg_cron retention/consent jobs and the daily B3-cancel Vercel cron fail silently (legal promises, withdrawn parent may get B3) | fix now | Fix (planned) (with OPS-04) |
| PERF-01 | performance / bundle | High | Measured | M | All 36 modules (≈800 KB gz) in first-load JS of every child screen; lesson LCP 10.8 s, topic list 8.4 s on a mid phone | fix now | Fix (planned) |
...
| SEC-02 | RLS | High | Reproduced | S | Viewer access cannot be revoked (learner_access DELETE → 42P17 for everyone); a hand-removed viewer can re-open the accepted invite | fix now | Fix (planned) |
...
| ARC-07 | test infra | Medium | Reproduced | S | A sync loop pins a vitest worker forever and orphans it when the run is killed (the "int hang" mechanism); CI has no job timeout | fix now | Fix (planned) |
...
| BUG-04 | progress sync / consent | Medium | Reproduced | S | P0C01 on a progress upload retried forever, blocking every child's uploads on the device, silently | fix now | Fix (planned) (with BUG-01) |
| BUG-05 | voice | Medium | Reproduced | S | One failed manifest fetch → browser TTS/silence for the whole session | fix now | Fix (planned) |
| BUG-08 | storage | Medium | Reproduced | M | IndexedDB-hang boot writes to localStorage; next boot orphans it (incl. the upload queue) | after beta | Fix (planned) |
| BUG-10 | error paths | Medium | Suspected | M | "Check your connection" shown for consent/RLS/expired-session failures (incl. right-to-correct) | after beta | Fix (planned) (classifier); wording → Needs Rafi N9 |
...
| FND-15 | compliance | Medium | Measured | M | Deletion audit trail missing (~1,440 rows deleted untracked) | after beta | Fix (planned) |
...
| MAP-02 | privacy / billing | Medium | Suspected | S | `billing_events.payload` stores the full Stripe event (parent email, name, address), so after account deletion the row still identifies the family, against `accountDeletion.ts`'s "stripped" claim | before billing | Fix (planned) |
...
| OPS-03 | alerting | Medium | Measured | S | red-main reports a failed `migrate-prod` as "not in production, the gate working" while the app is already live | fix now | Fix (planned) |
...
| OPS-08 | deploy / backup | Medium | Measured | S | No backup taken before `migrate-prod`; D6 margin was 23 s | fix now | Fix (planned) |
...
| OPS-10 | runbooks | Medium | Measured | S | Rollback runbook wrong for Hobby (auto-assign off after rollback, one step back only), relies on unbought PITR, and instructs prod `psql`/MCP writes | fix now | Fix (planned) |
...
| OPS-13 | supply chain | Medium | Measured | S | Third-party actions pinned by tag in jobs holding prod secrets; `allowed_actions: all` | after beta | Fix (planned) |
...
| PERF-03 | performance / data | Medium | Measured | S | /parent does 9 + 4 queries per child (teacher with 30 students: 72); lesson_progress read twice per child; dashboard RPC returns only emptied legacy tables | fix now | Fix (planned) (dedupe only; one-RPC version later) |
| SEC-04 | API | Medium | Suspected | S | /api/auth/signup is an anonymous email relay from noreply@radlor.com (per-instance IP limit only) | fix now | Fix (planned) |
...
| SEC-06 | logging | Medium | Suspected | S | Crash reports persist URL fragment/query credentials (consent token, token_hash, implicit-flow tokens) | fix now | Fix (planned) |
| SEC-07 | RLS/consent | Medium | Measured | S | learners DELETE policy bypasses delete_child_data; Classes rollback still uses it | after beta | Fix (planned) (client stops using DELETE; the policy drop is a later contract migration) |
| SEO-02 | social / OG | Medium | Measured | S | radlor.com/radlic has no og:image/twitter:image (only radlor.com page without); page's `openGraph` object replaces the inherited file-based image (mechanism Suspected) | fix now | Fix (planned) (radlor-site) |
...
| ARC-15 | docs | Low | Measured | S | Stale architecture doc, wrong "auto-generated" header, false "not synced" comment, 25 files naming deleted systems, ~141 KB teen docs | fix now | Fix (planned) (docs bundle) |
| ARC-16 | dead code | Low | Measured | S | Unused: zustand dep, 30 unreferenced exports incl. 3 whole files, 5 starter SVGs, a test that keeps a dead function alive | fix now | Fix (planned) |
| BUG-06 | analytics | Low | Reproduced | S | Analytics: in-flight events deleted unsent; one refused event blocks all later events | after beta | Fix (planned) (with MAP-07) |
| BUG-11 | service worker | Low | Suspected | S | SW activate wipes the content-addressed clip/image cache on every version bump | after beta | Fix (planned) (sw bundle) |
...
| MAP-07 | analytics | Low | Reproduced | S | `learner_events` flush erases events tracked while an upsert is in flight (`kv.remove` of the whole key) | after beta | Fix (planned) |
...
| OPS-12 | docs | Low | Measured | S | Docs say Supabase Free / secrets unset (stale); backup.yml says delete itself when PITR is on | fix now | Fix (planned) (docs bundle) |
...
| OPS-17 | backup | Low | Measured | S | Backup job trusts `PROD_PROJECT_REF` without `assert-prod-ref.sh` | after beta | Fix (planned) |
| OPS-18 | CI | Low | Measured | S | No `timeout-minutes` on CI/deploy jobs; a known vitest hang would hold the deploy queue 6 h | fix now | Fix (planned) (with ARC-07) |
| OPS-19 | frontend ops | Low | Measured | S | SW precaches removed routes; old open tabs can hit chunk 404s after takeover (no Skew Protection on Hobby) | later | Fix (planned) (sw bundle) |
...
| OPS-21 | monitoring | Low | Measured | S | `/api/health` is shallow (no Supabase/auth check, no version) | with OPS-04 | Fix (planned) (with OPS-04) |
...
| PERF-05 | performance / images | Low | Measured | S | /auth LCP is a 103 KB PNG shown 56 px tall; a 27 KB re-encode saves ≈0.4 s | after beta | Fix (planned) |
...
| PERF-11 | memory | Low | Suspected | S | Speech keep-alive setInterval reassigned without clearing | after beta | Fix (planned) |
| PERF-12 | performance / offline | Low | Measured | S | SW precaches deleted /profile and /shop (404) | later | Dup of OPS-19 |
| SEC-08 | RPC integrity | Low | Reproduced | S | A child's login can mint unlimited points via record_lesson_progress | later | Fix (planned) |
...
| SEC-11 | headers | Low | Measured | S | connect-src *.supabase.co wildcard, camera=(self) leftover; radlor.com sends no security headers | after beta | Fix (planned) (app half); radlor.com headers later |
...
| SEC-14 | docs | Low | Measured | S | docs/security.md stale (CSP, V13, DEFINER count, CI step, /api/lead) | fix now | Fix (planned) (docs bundle) |
...
| SEC-16 | RPC | Low | Reproduced | S | entitled_chapters/is_chapter_entitled answer for any learner (no access guard) | later | Fix (planned) |
| SEC-17 | API | Low | Measured | S | Error responses name missing env vars / echo signature errors | later | Fix (planned) |
| SEO-04 | index coverage | Low | Measured | S | `/demo` is indexable: 200, not disallowed, no noindex, inherits the home title/description, thin legacy "New lessons are on the way" page | after beta | Fix (planned) (noindex); delete route → Needs Rafi N17 |
| SEO-05 | social / OG | Low | Measured | S | `/help` (and every app page without its own `openGraph`) inherits `og:url=https://radlic.com` and the home og:title; radlic.com uses `twitter:card: summary` with a 1200×630 image | after beta | Fix (planned) |
