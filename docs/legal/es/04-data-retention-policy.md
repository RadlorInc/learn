...
**Los registros de plataforma del proveedor de base de datos.** Registran la dirección IP, el tipo de navegador y una ubicación aproximada derivada de la IP en cada solicitud, incluidas las solicitudes que hacen los dispositivos de los niños. Son registros propios del proveedor, no tablas nuestras, y ninguno de nuestros procesos de eliminación llega a ellos. [PLACEHOLDER — establish the provider's retention period for these logs and state it here; if it is configurable, configure it.]
