...
7. **When progress was produced (N33, 26 September 2026).** A fix to how two devices' progress is merged (review BUG-02, PR #243) stores, on each lesson-progress row, the time the child's answer that produced that standing was given (`lesson_progress.answered_at`) — the same kind of fact as the time the row was last updated, which is already stored. The notice's "answers to maths questions, scores, points, progress" row already covers it in our reading, so the notice text is unchanged; the founder's decision is to name it explicitly at the next notice change. Please confirm that reading.
