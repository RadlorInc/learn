...
| **Resend** | Delivers every email the service sends. The app sends its own emails through Resend directly — the sign-up email, the two consent emails and the cancellation notice — and Resend is also the SMTP relay behind the authentication service's mailer (password reset, change of email address). **Every message a parent receives is delivered by Resend** | **No**, unless a progress email is ever built that names a child — none exists | The parent's email address and the content of the message | United States: North Virginia, us-east-1 (confirmed from the Resend dashboard, 24 September 2026) | Service provider |
