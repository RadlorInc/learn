...
| **GitHub Actions** | Encrypted database backups, stored as a 30-day build artifact | **Would** — a backup contains everything | The whole database, encrypted | [PLACEHOLDER — confirm] | Service provider. **No backup artifact exists today: the job has failed 13 nights running. See the Security Program.** |
