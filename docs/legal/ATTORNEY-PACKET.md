...
- **⚠️ CHANGED 2026-09-25 (founder's decision, made after being told in writing that the second email is the "plus" of email-plus):** the second email is **no longer sent**. A parent who signs up with email and password gets ONE email that confirms their address and asks for consent (doc 03 B0); a Google sign-in parent gets B1; either way the consent is granted when the parent ticks the box on the page the link opens, and nothing follows. Consents are now recorded as method `email`, not `email_plus` (migration 20260926090000); older consents keep `email_plus` and their second email. Notice v7 no longer promises a second email.
- **Before that:** the second email was sent **24 hours** after the grant. Only email-plus is built; the payment-card method in doc 03 is not.
- **Decide, FIRST:** (0) **Is a single email the parent answers, with no delayed confirmation, a method of verifiable parental consent under § 312.5(b) for this service?** If not, what is the smallest addition that makes it one (restore the second email, or another method)? Until answered, every consent since 2026-09-25 is recorded as `email` so it can be found and re-asked.
