...
| ⚠️ B3 happened | #192 merged 24 Sep → **Deploy #428 red** (`5ab91bd`): a test harness from #189 could not route `consent_b3_due` (digits in the name). `promote` + `migrate-prod` skipped; **`20260923200000` NOT applied**. #194/#197 WERE applied (their `migrate-prod` logs: `180000`, `180100`, `190000`). | — | — | — | — |
| B3′ | [#201](https://github.com/RadlorInc/learn/pull/201) — fixes the harness **and** carries a comment-only note in `20260923200000`, so its merge push is the one that offers that migration to `migrate-prod` (`migrations-changed` diffs the push's own range; without the note it would skip). | `20260923200000` (unapplied from #192) | — | **2.7** A–F | Merge → Backup (green, finished, artifact) → approve `production-db`; the log must name **only** `20260923200000`. |
