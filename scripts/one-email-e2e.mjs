...
//
// ONE SIGN-UP EMAIL, STILL EMAIL-PLUS (founder, 2026-09-25): the real Next routes against a LOCAL Supabase stack.
// Per path: how many emails go out NOW (ours, at the Resend stand-in, and Supabase's own mailbox), and that B3 is
// SCHEDULED a day after the grant — and cancelled on withdrawal.
...
const sent = async to => ((await j(`${RESEND}/__calls`)).body ?? []).filter(c => c.path === '/emails' && c.body?.to?.[0] === to).map(c => c.body)
/** Delivered at once (not scheduled): what lands in the inbox today. */
const ours = async to => (await sent(to)).filter(m => !m.scheduled_at)
const scheduled = async to => (await sent(to)).filter(m => m.scheduled_at)
const cancels = async () => ((await j(`${RESEND}/__calls`)).body ?? []).filter(c => /\/cancel$/.test(c.path)).length
// Supabase's own emails: the stack's mailbox, MINUS anything from our sender (a local stand-in may copy ours into the
// same mailbox so a person can read them — counting those would call our email Supabase's).
const supabases = async to => ((await j(`${MAIL}/api/v1/search?query=${encodeURIComponent('to:' + to)}`)).body?.messages ?? [])
  .filter(m => m.From?.Address !== 'noreply@radlor.com').length
...
console.log('\n── A. a parent signs up with email + password: ONE email now, it confirms and asks; the grant schedules B3')
{
  ...
  ok(/Clicking the button also confirms your email address\./.test(m?.text ?? ''), 'it says the button also confirms the address')
  ...
  ok(c?.method === 'email_plus' && c.state === 'granted' && !!c.second_email_provider_id, `consent recorded as email-plus with its B3: ${JSON.stringify(c)}`)
  ok(c?.parent_ack_at === null, 'no on-screen tick is claimed for the sign-up')
  ok((await ours(email)).length === 1, `still one email delivered now (${(await ours(email)).length})`)
  const [b3] = await scheduled(email)
  const delay = b3 ? new Date(b3.scheduled_at).getTime() - Date.now() : 0
  ok(!!b3 && /permission you gave/.test(b3.subject), `B3 scheduled: "${b3?.subject}" at ${b3?.scheduled_at}`)
  ok(delay > 0, `B3 is in the future (${Math.round(delay / 60000)} min — the local run sets CONSENT_SECOND_NOTICE_DELAY_MINUTES; production refuses anything but 24–48 h)`)
  ok(await supabases(email) === 0, `Supabase sent nothing (${await supabases(email)})`)
  const before = await cancels()
  const w = await j(`${APP}/api/consent/respond`, { method: 'POST', headers: json, body: JSON.stringify({ t, action: 'withdraw' }) })
  ok(w.body?.status === 'withdrawn', `withdrawal (${w.body?.status})`)
  ok(await cancels() === before + 1, 'withdrawing cancels the scheduled B3')
  void url
}
...
console.log('\n── E. a Google-style parent (confirmed, no confirmation email): B1 from the dashboard, then B3 scheduled')
{
  ...
  ok((await ours(email)).length === 1 && (await scheduled(email)).length === 1, `B1 now, B3 scheduled (${(await ours(email)).length} + ${(await scheduled(email)).length})`)
  ok((await consents(email))[0]?.method === 'email_plus', 'recorded as email-plus')
}
