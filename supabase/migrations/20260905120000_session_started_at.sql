...
-- ⚠️ DROPPING THE DEFAULT IS WHAT MAKES THE OLD SIGNATURES HONEST, AND IT IS WHY THIS MIGRATION
-- REDEFINES NOTHING. `now()` was the entire defect: the 10- and 11-arg functions never mention
-- `started_at`, so the default silently filled it with the row's INSERT time and every row got a
-- fabricated "start" that was really an end. With no default those same untouched functions now
-- write NULL — "this client did not tell us" — and a browser holding an old bundle degrades to
-- honest silence instead of a plausible lie. It also covers the direct-INSERT path: `sessions:
-- parent can insert` lets a client write this table without the RPC at all.
...
-- ⚠️ THE OLDER SIGNATURES ARE DELIBERATELY LEFT UNTOUCHED. An earlier draft re-created the 11-arg
-- version as a forwarding shim, and `billingSchema.test.ts` caught it: that gate takes the LAST
-- definition of a function name across all migrations and requires it to keep every guard an
-- earlier one added, so a trailing shim — which correctly has no guards, because it delegates —
-- reads as a dropped `raise`. The gate is right that a `create or replace` losing a guard is how
-- V5's payload bounds were silently reverted. Not redefining them is both smaller and safer.
