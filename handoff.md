...
8. 🧹 **Local workspace, as left 2026-09-14:** `main` = live (`0ca08753`, PRs #103–#105 all merged). The original folder `milo-story-mode` is still on `adult-surface-responsive` (behind `main`) with **uncommitted leftovers of the Topic 1 art pilot** (`LessonPlayer.tsx`, `Pictures.tsx`, `grade3Module1.ts`, `script.ts`, `public/assets/lessons/`) — superseded by #103, safe to discard; that branch's classroom work is still unmerged. Worktree `../milo-lesson-art` is where this session's work happened (now detached at `main`; `.claude/launch.json` entry `lesson-art-dev` runs it on :3000). Worktree `../milo-newflow` and the local branches `lesson-art-pilot`, `new-flow-all-grades`, `handoff-grades-3-8-live` are merged and removable.
9. 📝 **How this session's work should be repeated:** new modules = one writer agent per module on AUTHORING.md + one blind solver per module on `scripts/lesson-questions.mjs`, then register in `content/index.ts` and run `lessonsAllModules.test.ts`. ⚠️ **Keep ~6 agents running at once** — 12 hit the account's usage limit and five writers died mid-file (resumed with SendMessage, not restarted). ⚠️ **When moving a handoff block, replace by exact paragraph boundaries and diff against git before saving** — a header edit that spanned from the top to the newest block deleted ~190 lines of standing notes this session; caught by the byte count, restored from git.
