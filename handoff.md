...
> 📚 **2026-09-14 — GRADES 3 TO 8, ALL 36 MODULES, WRITTEN AND GATED; A GENERAL LESSON ENGINE (fractions, decimals, negatives, times, choices, 21 diagram kinds); AND PARENTS CHOOSE TOPICS.** Branch `new-flow-all-grades` (worktree `../milo-lesson-art`) · `tsc` 0 · vitest **104 files, 2998 passed** · `next build` OK · **sw v185 → v186** · shipped as one PR from `new-flow-all-grades` (founder: "push and deploy to production"). ⚠️ **Migration `20260914120000_learner_lesson_ids.sql` is committed but applied NOWHERE — merging does not apply it:** `migrate-prod` needs `migrate-staging`, which is skipped (no `STAGING_PROJECT_REF`), measured on the last Deploy run (both jobs `skipped`). Before it the same day: PR [#103](https://github.com/RadlorInc/learn/pull/103) merged and live (sw v185) — Higgsfield art for Topic 1 (`public/assets/lessons`), no scroll on Screen 8, **← Back on Screens 2–8**.
...
`/parent/topics?learner=<id>` (button "📚 Choose topics" on `/parent`, owner only) → `TopicPicker.tsx`: "Every topic" or grade tabs → module/topic checkboxes → `setLearnerLessons` writes `learners.lesson_ids text[]` (null = every topic). The child's `/modules`, `/menu`, topic path and practice use `chosenModules`. Driven on the preview + a faked session learner: home showed only the chosen grades/modules, the path "0 of 2 done". ❌ **Never driven signed in, never against a database** — until the migration is applied, saving answers "needs a database update" (PGRST204 → `not_ready`). Expand-only, safe before or after the client. Only the creating parent can write it (existing `learners: update` policy).
...
2. 🔴 **Apply `20260914120000_learner_lesson_ids.sql`** to production by hand (`docs/runbooks/applying-migrations.md`), then save a real choice as a signed-in parent and open that child's home.
