...
> ships in `src/features/lessons/`. The recorded voice clips are DELETED.
> 🎨 **Since PR [#101](https://github.com/RadlorInc/learn/pull/101) the UI follows the founder's SampleUI template**
> (thick ink borders, coral bar, teal buttons): the child's home is **`/modules`** (also `/menu`) → Learn (`/lesson`
> topic path) or Practice (`/practice?module=g3m1`), and every answering screen has a **scratch pad**. The layout
> pieces are `Frame.tsx` (teaching screens) and `PracticeLayout.tsx` (Screen 8 + all practice) — **build new
> modules into those, do not re-lay-out a screen.** **Next: Module 2 scripts for review.** Story in the 🎨 block below.
...
## ▶ OPEN
1. 🔴 **Merge / deploy of #101 — re-measure, do not trust this line** (commands in the header of this block).
