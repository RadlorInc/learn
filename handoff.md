...
- **Later the same evening (branch `parent-assign-performance`, sw v199):** **Assign lessons** (child sees ONLY assigned topics, each with a due date — `learners.lesson_due`, migration `20260917114845` applied by hand, measured: column + CHECK present, policies still 4, API sees it) and **Performance** (mastered / finished / problems this week / first-try % over 30 days / "needs help with" = 6+ problems, not mastered, first try under 50% — no time practised, founder's call). Rules in `src/features/lessons/progressReport.ts`, tested with 8 planted breaks. Dashboard: Learners is its own tab (no scroll), counts per child with name chips, "Start learning" removed from the header and Quick actions, "Plan & billing" removed from the Topics card. Both parent screens driven only on a throwaway page with stub data; never signed in.
...
| [#118](https://github.com/RadlorInc/learn/pull/118) | `delete_my_account` removes children's logins · dead "Are you a student?" link (→ `/auth/child`, 404) removed · this block | v197 |
| [#119](https://github.com/RadlorInc/learn/pull/119) | CI: Supabase CLI pinned to **2.117.0** in all five `setup-cli` steps | — (CI only) |
...
- ⚠️ **#118's first deploy run went RED on `rls-tests` without running a single assertion:** `Failed to resolve latest Supabase CLI release: rate limit exceeded`. `promote` was skipped, so production correctly stayed on v196; a re-run of the failed job went green (74 RLS assertions) and promoted. **A red `rls-tests` with that message is NOT evidence about RLS.** Fixed at the root by #119 (pinned 2.117.0 — the version the green re-run installed; the PR's own CI log shows `version: 2.117.0` passed to setup-cli, and #119's deploy ran green first time). Upgrade the CLI by editing those five lines on purpose.
- ⚠️ **Vercel Deployment Storage was 60 GB on the 10 GB Hobby plan** (founder's screenshot, 2026-09-17; Functions Storage 7.17 / 10 GB). **Deleted 108 deployments** with `vercel remove` after listing them for the founder and re-checking live state right before: adaptivelearn 44 (27 merged-branch previews + 17 older production builds), website 35, radlor-ops 29. **Kept per project: the live production deployment + the one before it** (instant rollback). Measured after: 108 removed / 0 failed; adaptivelearn, website and radlor-ops have 2 left, fraction-lesson-demo and minemath 1; `adaptivelearn.radlor.com`, `radlor.com`, `ops.radlor.com` still point at the same live deployment ids; `/auth` answers 200. **The storage bar was NOT re-measured** (Vercel updates it with a lag; per-deployment size is not in the API). ⚠️ **It will refill:** every adaptivelearn deploy carries all ~7,300 voice clips and the art, and there were 17 production builds in one day — so it is Pro, a recurring cleanup, or moving `public/audio` out of the deployment (e.g. Supabase Storage).
- ⚠️ My many `curl` polls of the live site tripped **Vercel's bot checkpoint** for this machine's IP (HTML "Vercel Security Checkpoint", then a 403 on `sw.js` in the browser pane). Not a site outage: fetched from outside (Vercel MCP `web_fetch_vercel_url`) the site answered 200. Poll `vercel ls adaptivelearn --prod` instead of hammering the domain.
...
5b. 🔴 **Vercel storage will refill** — check the Deployment Storage bar dropped after the 108 deletions, then decide: Pro, a recurring cleanup (keep live + previous per project), or `public/audio` off the deployment.
...
10. ⏭️ **LIFTED from 🔢 2026-09-15 (archived):** founder read of g5m1 at `/lesson-preview?module=g5m1` (titles vs the photographed contents page) · tape-diagram text ~8px on a 375px phone in every module · the landing copy still describes the placement check, which is off · PR #95 (stop `main` previews) status unconfirmed.
