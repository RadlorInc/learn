...
> 🪜 **2026-09-17: practice is ADAPTIVE in all 36 modules — each level is a different KIND of question, every ladder agreed with a blind solver, and review brings weak topics back.** LIVE (PR #112, sw v192); every fixed lesson line now plays a recorded clip (PR #113, sw v193). No human has read a ladder. Story in the 🪜 block below.
...
> 🪜 **2026-09-17 — PRACTICE IS ADAPTIVE IN EVERY MODULE: A HARDER QUESTION IS A DIFFERENT KIND OF QUESTION, NOT BIGGER NUMBERS. ALL 36 MODULES / 282 TOPICS / 5 STYLES EACH, EVERY ONE CHECKED BY A BLIND SOLVER.** ✅ **LIVE — as measured 2026-09-17:** PR [#112](https://github.com/RadlorInc/learn/pull/112) merged `9514cb3b` (it also shipped `lesson-teacher-voice`: every lesson a teacher), Deploy green, `release` = `9514cb3b`, `sw.js` serves **v192**; on production g5m1-t12 practice climbed bare → story → missing → two-step and mastered after 8, g3m2-t1 fetched Teddy clips (206), 0 console errors. ✅ **Then PR [#113](https://github.com/RadlorInc/learn/pull/113) merged `1b1841f8`: the 2,527 queued clips, sw v193 — as measured 2026-09-17:** Deploy green, `release` = `1b1841f8`, `sw.js` serves **v193**, live manifests Teddy 3,846 / Stevie 3,477, g3m2-t1 Screen 1 on production played new clip `1gj7k5z` (206), 0 console errors. Re-measure: `curl -s https://adaptivelearn.radlor.com/sw.js | head -1`. Voice queue is **0** (`lessonVoiceClips.test.ts` caps it at 0; watched red at 2527 on the old manifests); the notebook clones `adaptive-practice`, which now equals `main` · `tsc` 0 · vitest **108 files, 3984 passed**, 20 skipped · `next build` OK · rules, gate semantics and the leak classes in **docs/new-flow/AUTHORING.md → "Practice is ADAPTIVE"**.
...
7. ✅ **Every lesson line with a fixed text plays a recorded clip (2026-09-17):** `public/audio/<voice id>/` holds Teddy 3,846 + Stevie 3,477 (zips `clips-lessons-20260916-145109` = beats + big ideas, `clips-lessons-20260917-020431` = Screen 1/8/9, hints, twin, feedback and the 27 Stevie lines once deduplicated into Teddy). `LessonPlayer` sets `setSceneVoice(lessonVoice(grade))`: Teddy in Grades 3–5, Stevie in 6–8. Practice QUESTIONS (generated numbers) are still browser speech — founder: not now. ⚠️ **Nobody has listened** to either batch on a device; only file sizes per character were checked (no truncation outliers against the first batch). ⚠️ A NEW or reworded line has no clip until the corpus is regenerated (`npx tsx scripts/lesson-voice-corpus.mts`), pushed to the branch the notebook clones (`BRANCH` in `scripts/chatterbox-kaggle.ipynb`), rendered, and merged — `lessonVoiceClips.test.ts` goes red if a spoken line is neither on disk nor queued.
