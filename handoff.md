...
> 2. ✅ **A full ten-round run HAS now been played — twice, from both ends — see §⑤b.** The night and
>    evening washes, the re-teach, the tier climbing to L3 with the ring retiring, and mastery
>    early-exit are all confirmed on screen. It found two more faults, both fixed and gated.
>    ⚠️ **Still not seen in a natural run: a "to" time or a 5-minute time.** Run B reached L3 and
>    mastered out on `quarter past`; the L3 content is still only verified via the forced `?d=3` drive
>    (where it was driven properly, including the 7:50 → "ten to 8" case both ways).
