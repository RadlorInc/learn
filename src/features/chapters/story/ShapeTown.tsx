 ...
 *
 * WHAT CHANGED AND WHY. This chapter used to be "a voice names a shape, tap it among three" — the same
 ...
 *
 * So the question is now a HOLE in the picture. A build is going up; one socket sits empty and pulsing;
 ...
 * instead: a house goes up in the garden, then the scene moves to the beach and a boat is built. That is
 ...
 * learned when a character, dropped on the flier band, ended up hovering in the treetops.
 *
...
export const BUILDS: BuildDef[] = [
  ...
  },
  {
    ...
    opening: 'Now let\'s build a boat!',
    parts: [
 ...
 * reason the build can never drift out of step with the round number. Step 0 is the worked
...
// ─── The pieces ──────────────────────────────────────────────────────────────────────
// A pile of parts on the ground to the left of the build, wrapping two to a row so three reads as
...
// ─── Watch it fit (opening demo + the 3-wrong re-teach) ────────────────────────────
/**
 * The demo picks the piece up and fits it — so the demo is a real build step, not a mime of one.
...
export function makeShapeBeat(fit: Fit): Beat<ShapeRound> {
  return {
    ...
    // One walk, on the round the second build starts — so the change of place reads as a
    // journey there rather than the backdrop simply swapping underneath.
...
export default function ShapeTown({ onFinish, onExit }: {
   ...
   * appeared with no word said or written about it. That is the beat a tester reported as the chapter not
  ...
  return (
    <div style={{ position: 'relative', width: '100vw', height: '100dvh', overflow: 'hidden' }}>
      ...
      {phase === 'intro' && (
        <div style={{ position: 'absolute', inset: 0, zIndex: 45, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 20 }}>
          <div style={{ maxWidth: '74%', background: '#fff', border: '3px solid var(--outline)', borderRadius: 18, padding: '14px 20px', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 19, color: 'var(--ink)', textAlign: 'center', boxShadow: '0 4px 0 rgba(61,37,22,.1)' }}>
            Let&apos;s build a house out of shapes! Every part needs the shape that fits. First, let&apos;s meet the shapes!
          </div>
      ...
      {phase === 'demo' && (<>{Banner('Watch the first piece fit')}
