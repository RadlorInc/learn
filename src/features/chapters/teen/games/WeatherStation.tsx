...
function colder(): Task {
  ...
  return {
    ...
    context: `One account is at ${disp(a)}, the other at ${disp(b)}. A balance below 0 means you owe money, so it counts as a lower balance than one above 0.`,
    padInstruction: `Tap the account with the ${low ? 'lower' : 'higher'} balance.`,
...
function afterChange(): Task {
  ...
  return {
    ...
    context: `Your balance starts at ${disp(s)}. You ${dir} — ${out ? 'money goes out, so the balance drops' : 'money comes in, so the balance rises'}.`,
    padInstruction: 'Work out where the balance lands, then tap that number.',
...
function opposite(): Task {
  ...
  return {
    ...
    context: `Every number has an opposite: the same distance from 0, but on the other side of it. So flip ${disp(t)} across 0.`,
    padInstruction: `Tap the opposite of ${disp(t)}.`,
...
function distance(): Task {
  ...
  return {
    ...
    context: `Distance from 0 just means how many steps away a number is — never which way, so it is never negative. How far is ${disp(t)} from 0?`,
    padInstruction: 'Tap how many steps it is.',
    answer: ans, start: 0, choices: numChoices(ans, t),
...
const GUIDED_TASK: Task = {
  ...
  context: 'Your balance starts at −1. You withdraw 4 — money goes out, so the balance drops below −1.',
  padInstruction: 'Work out where the balance lands, then tap that number.',
