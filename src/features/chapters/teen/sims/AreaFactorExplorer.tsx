 ...
 * data-band scope; colors/fonts via CSS variables only.
 */
...
export default function AreaFactorExplorer({ band, onReady }: AreaFactorExplorerProps) {
  ...
  const tiles = [
    ...
    { x: pad + xPx, y: pad + xPx, w: aPx, h: bPx, fill: 'var(--accent)', op: 0.55, label: disp(a * b) },
  ]
    ...
    <SimLayout maxWidth={380} gap={16} align="center" visual={<>
      ...
      <div style={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
        ...
        >
          ...
          <text x={pad + xPx + aPx / 2} y={pad - 4} textAnchor="middle" fontSize={11} fill="var(--ink-muted)">{disp(a)}</text>
          <text x={pad - 4} y={pad + xPx / 2} textAnchor="end" dominantBaseline="middle" fontSize={11} fill="var(--ink-muted)">x</text>
          <text x={pad - 4} y={pad + xPx + bPx / 2} textAnchor="end" dominantBaseline="middle" fontSize={11} fill="var(--ink-muted)">{disp(b)}</text>
      ...
      <p style={{ margin: 0, textAlign: 'center', fontFamily: 'var(--font-body)', fontSize: 14, lineHeight: 1.5, color: 'var(--ink-soft)' }}>
        <strong style={{ color: 'var(--ink)' }}>Middle term {disp(a + b)}:</strong> it&apos;s a + b, the two strips.<br />
        <strong style={{ color: 'var(--ink)' }}>Constant {disp(a * b)}:</strong> it&apos;s a × b, the corner.
      </p>
    </SimLayout>
  )
}
