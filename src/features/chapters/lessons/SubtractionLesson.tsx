  ...
  return (
    <div className="milo-lesson" style={{minHeight:'100dvh',display:'flex',flexDirection:'column',alignItems:'center',background:'var(--bg-page)',padding:'10px 14px 24px',gap:10}}>
      ...
      <div style={{display:'flex',alignItems:'flex-end',gap:10,width:'100%',maxWidth:520}}>
        <img src={src} alt="Milo" loading="lazy" decoding="async" style={{width:66,height:66,objectFit:'contain',flexShrink:0,filter:'drop-shadow(0 4px 8px rgba(61,37,22,.2))',animation:miloMood==='celebrate'?'s_jump 0.7s ease-in-out infinite':'s_idle 3s ease-in-out infinite'}} onError={e=>{(e.target as HTMLImageElement).style.display='none'}}/>
...
export default function SubtractionLesson({childName,onLessonComplete}:Props){
  ...
  function next(){
    ...
    if(step>=TOTAL_STEPS-1){
      speak(`Brilliant, ${childName}! You can take away! Let's practice now!`)
