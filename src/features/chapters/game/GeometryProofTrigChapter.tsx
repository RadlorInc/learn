'use client'
/**
 * GeometryProofTrigChapter (15–16, "Commission" / studio analyst) —
 * Geometry chapter #12: Triangles, Proof & Right-Triangle Trig.
 *
 * Portal pattern (mirrors IntegersChapter / LinearEquationsInequalitiesChapter):
 * renders full-screen over the game stage so it bypasses the kids' zoom/celebration,
 * sets data-band="15-16" to scope the dark teen theme, and calls finishAndSync
 * itself. Flow:
 *   intro (CaseCard) → explore (TrigTriangleExplorer) → lesson → practice → done
 ...
 *   L1 (angle relationships)         → NumericEntry ("°")
 *   L2 (congruence/similarity proof) → StepSelect ("pick the next statement+reason")
 *   L3 (SOH-CAH-TOA side / angle)    → ChoiceGrid (rounded/irrational-safe),
 *       with a FigureDiagram of the right triangle for context.
 */
...
import StepSelect from '@/features/chapters/teen/StepSelect'
import NumericEntry, { numericEqual } from '@/features/chapters/teen/NumericEntry'
import FigureDiagram from '@/features/chapters/teen/FigureDiagram'
...
import GeometryProofTrigTeenLesson, {
  makeRound, ProofWatch, type Round,
} from '@/features/chapters/lessons/GeometryProofTrigTeenLesson'
...
function ProofTrigWorld({
    ...
    return (
      <Centered>
        <div style={{ width: '100%', maxWidth: 520 }}>
          <CaseCard
            band={BAND}
            title="The Ramp Inspection"
            why="A wheelchair ramp rises 0.8 m over a 9.6 m run — code caps the angle of elevation at 5°. You can't protract a ramp from a photo, but you can compute its angle from two measured sides."
            question="How do you find missing angles and sides — and prove two shapes are truly identical?"
  ...
  if (phase === 'explore') {
    return (
      <ExploreStep
        ...
        intro="Drag the angle and watch the opposite, adjacent, and hypotenuse re-label — and the three ratios update with them. The ratio depends only on the angle."
  ...
  if (phase === 'lesson') {
    return <GeometryProofTrigTeenLesson band={BAND} childName={childName} onLessonComplete={() => setPhase('practice')} />
  }
...
function ProofTrigPractice({
  ...
  // selected: for ChoiceGrid this is the choice value; for StepSelect it's the picked index.
  ...
  // ChoiceGrid: answer is the correct choice index.
  ...
  if (round.kind === 'numeric') {
    surface = (
      <div style={{ display: 'flex', justifyContent: 'center', width: '100%' }}>
        <NumericEntry band={BAND} status={status} onSubmit={(v) => submitNumeric(v)} placeholder="x" suffix={round.suffix} allowDecimal={false} />
      </div>
    )
  ...
  return (
    <div className="milo-lesson" style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', background: 'var(--bg-page)', color: 'var(--ink)', fontFamily: 'var(--font-body)' }}>
      <div style={{ width: '100%', maxWidth: 640 }}>
        <TeenTopbar band={BAND} title="Triangles, Proof & Trig" roundIdx={roundIdx} totalRounds={TOTAL_ROUNDS} onBack={() => { stopSpeech(); onExit() }} />
      </div>
      ...
      <main style={{ flex: 1, width: '100%', maxWidth: 560, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 20, padding: '12px 18px 28px', boxSizing: 'border-box' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, width: '100%' }}>
          ...
          <div style={{ flex: 1, background: 'var(--paper)', border: '1px solid var(--outline)', borderRadius: 12, padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <span style={{ fontFamily: 'var(--font-numeric)', fontVariantNumeric: 'tabular-nums', fontSize: 18, fontWeight: 700, color: 'var(--ink)', lineHeight: 1.4 }}>
              {round.promptText}
            </span>
          </div>
        </div>

        {/* Optional geometry figure for context (trig / triangle rounds). */}
        {round.figure && (
          <div style={{ display: 'flex', justifyContent: 'center', width: '100%' }}>
            <FigureDiagram band={BAND} kind={round.figure.kind} labels={round.figure.labels} highlight={round.figure.highlight} />
          </div>
        )}
...
// concept as a short step ladder, then continues (no penalty, no red).
function ReteachPanel({ round, onContinue }: { round: Round; onContinue: () => void }) {
  const [ready, setReady] = useState(false)
  if (typeof document === 'undefined') return null
  // Build a short ladder from the round's own pieces.
  const ladder = round.kind === 'step'
    ? [...(round.shown ?? []).map((s) => s.text), (round.options ?? [])[round.answer]?.text ?? '✓']
    : [round.promptText, round.kind === 'numeric' ? `x = ${round.answer}${round.suffix ?? ''}` : `${round.choices[round.answer]?.label ?? '✓'}`]
  return createPortal(
    <div data-band={BAND} role="dialog" aria-modal="true" style={{ position: 'fixed', inset: 0, zIndex: 950, background: 'color-mix(in srgb, var(--ink) 30%, transparent)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: 'var(--paper)', border: '1px solid var(--outline)', borderRadius: 16, padding: '22px 22px 20px', maxWidth: 520, width: '100%', boxShadow: '0 6px 28px color-mix(in srgb, var(--ink) 18%, transparent)' }}>
        ...
        <ProofWatch lines={[round.explain]} steps={ladder} onDone={() => setReady(true)} />
