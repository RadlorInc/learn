'use client'
/**
 * ComplexNumbersChapter (17–18, "Field Lab") — the Complex Numbers module.
 *
 ...
 * itself. Flow:
 *   intro (CaseCard) → explore (ComplexPlaneExplorer) → lesson → practice → done (MasteryState)
...
import ComplexNumbersTeenLesson, { makeRound, ComplexWatch, fmtInt, type Round } from '@/features/chapters/lessons/ComplexNumbersTeenLesson'
...
// ── Explore sim: a lightweight complex plane — plot a + bi with sliders ─────
function ComplexPlaneExplorer() {
  ...
  const label = b === 0 ? fmtInt(a) : a === 0 ? `${b === 1 ? '' : b === -1 ? '−' : fmtInt(b)}i` : `${fmtInt(a)} ${b < 0 ? '−' : '+'} ${Math.abs(b) === 1 ? 'i' : `${Math.abs(b)}i`}`

  const Slider = ({ text, value, onChange }: { text: string; value: number; onChange: (n: number) => void }) => (
    <label style={{ display: 'flex', alignItems: 'center', gap: 12, width: '100%', fontFamily: 'var(--font-body)' }}>
      ...
      <input type="range" min={-6} max={6} step={1} value={value} onChange={(e) => onChange(Number(e.target.value))} style={{ flex: 1, accentColor: 'var(--accent)', cursor: 'pointer' }} aria-label={text} />
      <span style={{ width: 40, textAlign: 'right', fontFamily: 'var(--font-numeric)', fontVariantNumeric: 'tabular-nums', fontSize: 16, fontWeight: 600, color: 'var(--accent)' }}>{fmtInt(value)}</span>
    </label>
  )
...
function ComplexNumbersWorld({
      ...
      <Centered>
        <div style={{ width: '100%', maxWidth: 640 }}>
          <CaseCard
            ...
            why="When a square root goes negative — in AC circuits, signals and quantum states — numbers step off the line onto a second axis."
            question="How do we add, multiply and measure complex numbers?"
            startLabel="Step off the line"
  ...
  if (phase === 'explore') {
    return (
      <ExploreStep
        band={BAND}
        title="Plot on the plane"
        intro="Slide the real and imaginary parts and watch the point move on the complex plane — and its modulus, the distance from the origin, update with it. Get a feel for it, then continue."
  ...
  if (phase === 'lesson') {
    return <ComplexNumbersTeenLesson band={BAND} childName={childName} onLessonComplete={() => setPhase('practice')} />
  }
  ...
  if (phase === 'done') {
    return (
      <Centered>
        <MasteryState
          band={BAND}
          conceptsConfirmed={['i and powers of i', 'Adding & subtracting', 'Multiplying', 'Modulus & the plane']}
          nextPointer="Next: rational functions."
...
function ComplexNumbersPractice({
  ...
  return (
    <div className="milo-lesson" style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', background: 'var(--bg-page)', color: 'var(--ink)', fontFamily: 'var(--font-body)' }}>
      <div style={{ width: '100%', maxWidth: 640 }}>
        <TeenTopbar band={BAND} title="Complex Numbers" roundIdx={roundIdx} totalRounds={TOTAL_ROUNDS} onBack={() => { stopSpeech(); onExit() }} />
      </div>
      ...
      <main style={{ flex: 1, width: '100%', maxWidth: 560, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 20, padding: '12px 18px 28px', boxSizing: 'border-box' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, width: '100%' }}>
          ...
          <div style={{ flex: 1, background: 'var(--paper)', border: '1px solid var(--outline)', borderRadius: 12, padding: '10px 14px', fontFamily: 'var(--font-numeric)', fontWeight: 600, fontSize: 16, color: 'var(--ink)' }}>
            {round.promptText}
          </div>
        </div>
  ...
  return createPortal(
    <div data-band={BAND} role="dialog" aria-modal="true" style={{ position: 'fixed', inset: 0, zIndex: 950, background: 'color-mix(in srgb, var(--ink) 30%, transparent)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: 'var(--paper)', border: '1px solid var(--outline)', borderRadius: 16, padding: '22px 22px 20px', maxWidth: 520, width: '100%', boxShadow: '0 6px 28px color-mix(in srgb, var(--ink) 18%, transparent)' }}>
        ...
        <ComplexWatch lines={[round.explain]} plane={round.plane} onDone={() => setReady(true)} />
...
export default function ComplexNumbersChapter(_props: Props) {
  ...
  const finish = useCallback((c: number, w: number, mastered?: boolean) => {
    ...
    finishAndSync('complexNumbers', c, w, 'practice', mastered)
  }, [finishAndSync])
