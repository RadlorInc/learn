'use client'
/**
 * IntroCalculusChapter (17–18, "Field Lab") — the Intro to Calculus module.
 *
 * Portal pattern (like FunctionToolkitChapter): renders full-screen over the
 * game stage, sets data-band="17-18" to scope the teen theme, and calls
 * finishAndSync itself. Flow:
 *   intro (CaseCard) → explore (secant→tangent sim) → lesson → practice → done
...
import IntroCalculusTeenLesson, {
  makeRound, CalcWatch, SecantTangent, secantSlopeInfo, type Round,
} from '@/features/chapters/lessons/IntroCalculusTeenLesson'
...
// ── The Explore sim: a live secant → tangent slider ─────────────────────────
...
function IntroCalculusWorld({
      ...
      <Centered>
        <div style={{ width: '100%', maxWidth: 640 }}>
          <CaseCard
            band={BAND}
            title="The Instant"
            why="Speedometers, growth rates and optimization all ask the same question: how fast is something changing at a single instant?"
            question="How do limits turn average rates into an instantaneous rate — the derivative?"
            startLabel="Zoom in"
  ...
  if (phase === 'explore') {
    return (
      <ExploreStep
        band={BAND}
        title="Zoom in on the instant"
        intro="Slide the second point Q toward the fixed point P and watch the secant slope close in on the tangent slope. The value it approaches is the derivative — the instantaneous rate of change."
  ...
  if (phase === 'lesson') {
    return <IntroCalculusTeenLesson band={BAND} childName={childName} onLessonComplete={() => setPhase('practice')} />
  }
  ...
  if (phase === 'done') {
    return (
      <Centered>
        <MasteryState
          band={BAND}
          conceptsConfirmed={['Limits by substitution', 'Average rate of change', 'Secant → tangent', 'Derivative & the power rule']}
          nextPointer="You've reached the top of the ladder — Module complete."
...
function IntroCalculusPractice({
  ...
  return (
    <div className="milo-lesson" style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', background: 'var(--bg-page)', color: 'var(--ink)', fontFamily: 'var(--font-body)' }}>
      <div style={{ width: '100%', maxWidth: 640 }}>
        <TeenTopbar band={BAND} title="Intro to Calculus" roundIdx={roundIdx} totalRounds={TOTAL_ROUNDS} onBack={() => { stopSpeech(); onExit() }} />
      </div>
  ...
  return createPortal(
    <div data-band={BAND} role="dialog" aria-modal="true" style={{ position: 'fixed', inset: 0, zIndex: 950, background: 'color-mix(in srgb, var(--ink) 30%, transparent)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: 'var(--paper)', border: '1px solid var(--outline)', borderRadius: 16, padding: '22px 22px 20px', maxWidth: 520, width: '100%', boxShadow: '0 6px 28px color-mix(in srgb, var(--ink) 18%, transparent)' }}>
        ...
        <CalcWatch lines={[round.explain]} onDone={() => setReady(true)} />
...
export default function IntroCalculusChapter(_props: Props) {
  ...
  const finish = useCallback((c: number, w: number, mastered?: boolean) => {
    ...
    finishAndSync('introCalculus', c, w, 'practice', mastered)
  }, [finishAndSync])
