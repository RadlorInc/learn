'use client'
/**
 * ExponentsPolynomialsChapter (15–16, "Field Lab" / Commission) — teen chapter.
 *
 * EXTENDS multiplication into algebra: exponent laws → zero/negative exponents
 * + scientific notation → polynomial add/subtract and binomial multiplication
 * (FOIL, shown as an area model, answered by MCQ).
 *
 ...
 * to scope the dark teen theme, and calls finishAndSync itself. Flow:
 *   intro (CaseCard) → explore (PowerExplorer) → lesson → practice → done
...
import ExponentsPolynomialsTeenLesson, {
  makeRound, AreaModel, ExponentsPolynomialsWatch, type Round,
} from '@/features/chapters/lessons/ExponentsPolynomialsTeenLesson'
...
function ExponentsPolynomialsWorld({
    ...
    return (
      <Centered>
        <div style={{ width: '100%', maxWidth: 520 }}>
          <CaseCard
            band={BAND}
            title="Scaling the Lab"
            why="Bacteria, data, and brightness all grow by repeated multiplying — and engineers fold whole expressions into one. Exponents and polynomials are the language for both."
            question="How do the exponent laws compress growth, and how do you multiply two expressions at once?"
  ...
  if (phase === 'explore') {
    return (
      <ExploreStep
        ...
        intro="Drag the base and the exponent, and watch the repeated multiplication and the value move together. Get a feel for how fast powers climb, then continue."
  ...
  if (phase === 'lesson') {
    return <ExponentsPolynomialsTeenLesson band={BAND} childName={childName} onLessonComplete={() => setPhase('practice')} />
  }
  ...
  if (phase === 'done') {
    return (
      <Centered>
        <MasteryState
          band={BAND}
          conceptsConfirmed={['Exponent laws (×, ÷, power)', 'Zero & negative exponents', 'Scientific notation', 'Add, subtract & multiply polynomials']}
  ...
  return (
    <div className="milo-lesson" style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', background: 'var(--bg-page)', color: 'var(--ink)', fontFamily: 'var(--font-body)' }}>
      <div style={{ width: '100%', maxWidth: 640 }}>
        <TeenTopbar band={BAND} title="Exponents & Polynomials" roundIdx={roundIdx} totalRounds={TOTAL_ROUNDS} onBack={() => { stopSpeech(); onExit() }} />
      </div>
      ...
      <main style={{ flex: 1, width: '100%', maxWidth: 560, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 20, padding: '12px 18px 28px', boxSizing: 'border-box' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, width: '100%' }}>
          ...
          <div style={{ flex: 1, background: 'var(--paper)', border: '1px solid var(--outline)', borderRadius: 12, padding: '10px 14px', fontFamily: 'var(--font-numeric)', fontWeight: 600, fontSize: 17, color: 'var(--ink)' }}>
            {round.promptText}
          </div>
        </div>

        {round.area && (
          <AreaModel area={round.area} />
        )}
...
// concept (with the area model when relevant), then continues (no penalty, no red).
  ...
  return createPortal(
    <div data-band={BAND} role="dialog" aria-modal="true" style={{ position: 'fixed', inset: 0, zIndex: 950, background: 'color-mix(in srgb, var(--ink) 30%, transparent)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: 'var(--paper)', border: '1px solid var(--outline)', borderRadius: 16, padding: '22px 22px 20px', maxWidth: 520, width: '100%', boxShadow: '0 6px 28px color-mix(in srgb, var(--ink) 18%, transparent)' }}>
        ...
        <ExponentsPolynomialsWatch lines={[round.explain]} area={round.area} onDone={() => setReady(true)} />
