'use client'
/**
 * RadicalsPythagoreanChapter (15–16, "Field Lab") — teen chapter #8.
 *
 * Portal pattern (like IntegersChapter): renders full-screen over the game stage,
 * sets data-band="15-16" (dark skin) to scope the teen theme, and calls
 * finishAndSync itself. Flow:
 *   intro (CaseCard) → explore (PythagorasExplorer) → lesson → practice → done (MasteryState)
 ...
 *
 * Answer surfaces: ChoiceGrid for radical-form / interval answers, NumericEntry
 * (tolerance) for clean-decimal Pythagoras answers.
 */
...
import RadicalsPythagoreanTeenLesson, {
  makeRound, isCorrect, RadicalWatch, type Round,
} from '@/features/chapters/lessons/RadicalsPythagoreanTeenLesson'
...
function RadicalsWorld({
    ...
    return (
      <Centered>
        <div style={{ width: '100%', maxWidth: 520 }}>
          <CaseCard
            band={BAND}
            title="The Diagonal Cut"
            why="A carpenter bracing a frame, a surveyor crossing a field, a screen measured corner-to-corner — the slanted distance never lines up with the grid, and a square root is how you find it."
            question="How do square roots work, and how do you find a missing side of a right triangle?"
  ...
  if (phase === 'explore') {
    return (
      <ExploreStep
        ...
        intro="Drag the two legs and watch the square on each side. The two small squares always add up to the square on the slanted side — that is the whole theorem. Get a feel for it, then continue."
  ...
  if (phase === 'lesson') {
    return <RadicalsPythagoreanTeenLesson band={BAND} childName={childName} onLessonComplete={() => setPhase('practice')} />
  }
  ...
  if (phase === 'done') {
    return (
      <Centered>
        <MasteryState
          band={BAND}
          conceptsConfirmed={['Square roots & estimation', 'Simplifying radicals', 'Adding like radicals', 'Pythagorean theorem & grid distance']}
...
function RadicalsPractice({
  ...
  function grade(v: string | number) {
    if (selected !== null) return
    const ok = isCorrect(round, v)
  ...
  return (
    <div className="milo-lesson" style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', background: 'var(--bg-page)', color: 'var(--ink)', fontFamily: 'var(--font-body)' }}>
      <div style={{ width: '100%', maxWidth: 640 }}>
        <TeenTopbar band={BAND} title="Radicals & the Pythagorean Theorem" roundIdx={roundIdx} totalRounds={TOTAL_ROUNDS} onBack={() => { stopSpeech(); onExit() }} />
      </div>
    ...
    <div data-band={BAND} role="dialog" aria-modal="true" style={{ position: 'fixed', inset: 0, zIndex: 950, background: 'color-mix(in srgb, var(--ink) 30%, transparent)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: 'var(--paper)', border: '1px solid var(--outline)', borderRadius: 16, padding: '22px 22px 20px', maxWidth: 520, width: '100%', boxShadow: '0 6px 28px color-mix(in srgb, var(--ink) 18%, transparent)' }}>
        ...
        <RadicalWatch
          lines={[round.explain]}
          visual={
            <div style={{ fontFamily: 'var(--font-numeric)', fontSize: 18, fontWeight: 600, color: 'var(--accent)', padding: '12px 18px', background: 'var(--bg-1)', border: '1px solid var(--outline)', borderRadius: 10, textAlign: 'center' }}>
              {round.promptText}
            </div>
          }
