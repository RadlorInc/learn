'use client'
/**
 * GeometryMeasurementChapter (12–14, "Field Lab") — "Area, Volume & the
 * Pythagorean Theorem" (id geometryMeasurement, EXTEND ← areaPerimeter).
 *
 ...
 *   intro (CaseCard) → lesson (GeometryMeasurementTeenLesson) → practice → done (MasteryState)
 ...
 *
 * Rounds mix two answer surfaces: NumericEntry (compute area/volume/missing side)
 * and ChoiceGrid (pick the right formula). FigureDiagram shows the labelled figure.
 */
...
import NumericEntry from '@/features/chapters/teen/NumericEntry'
...
import AreaExplorer from '@/features/chapters/teen/sims/AreaExplorer'
import GeometryMeasurementTeenLesson, {
  makeRound, gradeNumeric, FigureWatch, type Round,
} from '@/features/chapters/lessons/GeometryMeasurementTeenLesson'
...
// Spoken form of an answer (numbers read plainly; formula strings read as-is).
function spokenAnswer(r: Round): string {
  return typeof r.answer === 'number' ? String(r.answer) : r.answer
}
...
function GeometryWorld({
    ...
    return (
      <Centered>
        <div style={{ width: '100%', maxWidth: 520 }}>
          <CaseCard
            band={BAND}
            title="The Skate Ramp Build"
            why="Builders, packers and designers all measure: how much paint covers a wall, how much fits in a box, how long a brace must be."
            question="How do you find area, volume, and a missing side of a right triangle?"
  ...
  if (phase === 'explore') {
    return (
      <ExploreStep
        band={BAND}
        title="Resize the shape"
        intro="Drag the base and height sliders and watch how a rectangle's area (b×h) and a triangle's area (½×b×h) change together."
        onContinue={() => setPhase('lesson')}
      >
        <AreaExplorer band={BAND} />
      </ExploreStep>
    )
  }

  if (phase === 'lesson') {
    return <GeometryMeasurementTeenLesson band={BAND} childName={childName} onLessonComplete={() => setPhase('practice')} />
  }
  ...
  if (phase === 'done') {
    return (
      <Centered>
        <MasteryState
          band={BAND}
          conceptsConfirmed={['Area of polygons', 'Circle area & circumference', 'Prism volume', 'The Pythagorean theorem']}
          nextPointer="You've finished the 12-14 investigations!"
...
function GeometryPractice({
  ...
  // Shared resolution for both answer surfaces.
  ...
  function pickChoice(v: string | number) {
    resolve(v, v === round.answer)
  }

  // NumericEntry (compute) rounds.
  function submitNumeric(v: number) {
    resolve(v, typeof round.answer === 'number' && gradeNumeric(v, round.answer))
  }
  ...
  return (
    <div className="milo-lesson" style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', background: 'var(--bg-page)', color: 'var(--ink)', fontFamily: 'var(--font-body)' }}>
      <div style={{ width: '100%', maxWidth: 640 }}>
        <TeenTopbar band={BAND} title="Area, Volume & Pythagoras" roundIdx={roundIdx} totalRounds={TOTAL_ROUNDS} onBack={() => { stopSpeech(); onExit() }} />
      </div>
...
// concept on the labelled figure, then continues (no penalty, no red).
  ...
  return createPortal(
    <div data-band={BAND} role="dialog" aria-modal="true" style={{ position: 'fixed', inset: 0, zIndex: 950, background: 'color-mix(in srgb, var(--ink) 30%, transparent)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: 'var(--paper)', border: '1px solid var(--outline)', borderRadius: 16, padding: '22px 22px 20px', maxWidth: 520, width: '100%', boxShadow: '0 6px 28px color-mix(in srgb, var(--ink) 18%, transparent)' }}>
        ...
        <FigureWatch
          lines={[round.explain]}
          kind={round.figure.kind}
          labels={round.figure.labels}
          highlight={round.figure.highlight}
...
export default function GeometryMeasurementChapter(_props: Props) {
