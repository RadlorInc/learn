'use client'
/**
 * DataGraphsChapter (9–11) — read bar charts and pictographs (tallest, how-many, compare,
 * total) — as the DataDeck pre-teen "Number Lab" experience (see story/DataDeck.tsx +
 ...
 * Reuses skill `dataGraphs`.
 */
import { createPortal } from 'react-dom'
import { useEffect, useState, useRef, useCallback } from 'react'
import { useRouter } from 'next/navigation'
import DataDeck from '@/features/chapters/story/DataDeck'
import { useChapterSync } from '@/data/supabase/useChapterSync'
import CelebrationModal from '@/shared/ui/CelebrationModal'

export default function DataGraphsChapter(_props: { onComplete: (correct: number, wrong: number) => void; childName: string }) {
  ...
  const finish = useCallback((correct: number, wrong: number, mastered?: boolean) => {
    ...
    finishAndSync('dataGraphs', correct, wrong, 'practice', mastered)
  }, [finishAndSync])
  ...
  return createPortal(
    <div style={{ position: 'fixed', inset: 0, zIndex: 900, background: '#0a1026' }}>
      <DataDeck key={runKey} onFinish={finish} onExit={() => router.push('/menu')} />
