'use client'
/**
 * SignedRationalOpsChapter (12–14, "Field Lab") — Operations with Signed
 * Rational Numbers. Same portal pattern as IntegersChapter: renders full-screen
 * over the game stage, sets data-band="12-14", calls finishAndSync itself. Flow:
 *   intro (CaseCard) → lesson (SignedRationalOpsTeenLesson) → practice → done
 * Uses useAdaptive (L1/L2/L3). Rounds mix MCQ (ChoiceGrid) and numeric entry
 * (NumericEntry) per the round's kind; a number line backs +/− rounds in read
 * mode. No visible difficulty tier (locked rule).
 */
...
import SignedRationalOpsTeenLesson, {
  makeRound, SignedWatch, fmtNum, spoken, type Round,
} from '@/features/chapters/lessons/SignedRationalOpsTeenLesson'
...
function SignedRationalOpsWorld({
      ...
      <Centered>
        <div style={{ width: '100%', maxWidth: 520 }}>
          <CaseCard
            band={BAND}
            title="In the Red"
            why="Bank balances, game economies and temperature swings all add and subtract across zero — and the sign rules decide every total."
            question="How do you add, subtract, multiply and divide with negative numbers — and not flip the sign by mistake?"
  ...
  if (phase === 'explore') {
    return (
      <ExploreStep
        ...
        intro="Set a start and a jump, then watch the arrow move right for a positive and left for a negative to land on the sum."
  ...
  if (phase === 'lesson') {
    return <SignedRationalOpsTeenLesson band={BAND} childName={childName} onLessonComplete={() => setPhase('practice')} />
  }
    ...
    return (
      <Centered>
        <MasteryState
          band={BAND}
          conceptsConfirmed={[
            'Add & subtract across zero',
            'Subtract a negative = add',
            'Sign rules for × and ÷',
            'Signed fractions & decimals',
          ]}
          nextPointer="Next: fraction & decimal operations."
...
function SignedRationalOpsPractice({
  ...
  // Shared grading path for both MCQ and numeric answers.
  function grade(ok: boolean, shown: string | number) {
    if (selected !== null) return
    setSelected(shown); setStatus(ok ? 'correct' : 'wrong')
    const res = ada.record(ok)
    const run = ok ? 0 : wrongRun + 1
    setWrongRun(run)
    if (ok) { setCorrect((c) => c + 1); speak(`Correct. ${ada.praise}`) }
    else { setWrong((w) => w + 1); speak(`The answer is ${spoken(round.answer)}. ${ada.encouragement}`) }
  ...
  function submitNumeric(value: number) {
    grade(numericEqual(value, round.answer, 1e-6), value)
  }
  ...
  return (
    <div className="milo-lesson" style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', background: 'var(--bg-page)', color: 'var(--ink)', fontFamily: 'var(--font-body)' }}>
      <div style={{ width: '100%', maxWidth: 640 }}>
        <TeenTopbar band={BAND} title="Operations with Signed Numbers" roundIdx={roundIdx} totalRounds={TOTAL_ROUNDS} onBack={() => { stopSpeech(); onExit() }} />
      </div>
      ...
      <main style={{ flex: 1, width: '100%', maxWidth: 560, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 20, padding: '12px 18px 28px', boxSizing: 'border-box' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, width: '100%' }}>
          ...
          <div style={{ flex: 1, background: 'var(--paper)', border: '1px solid var(--outline)', borderRadius: 12, padding: '10px 14px', fontFamily: 'var(--font-numeric)', fontWeight: 700, fontSize: 18, color: 'var(--ink)' }}>
            {round.promptText}
          </div>
        </div>
...
// concept (on a number line when one fits, else the worked expression), then
    ...
    <div data-band={BAND} role="dialog" aria-modal="true" style={{ position: 'fixed', inset: 0, zIndex: 950, background: 'color-mix(in srgb, var(--ink) 30%, transparent)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: 'var(--paper)', border: '1px solid var(--outline)', borderRadius: 16, padding: '22px 22px 20px', maxWidth: 520, width: '100%', boxShadow: '0 6px 28px color-mix(in srgb, var(--ink) 18%, transparent)' }}>
        ...
        <SignedWatch
          ...
          min={line?.min}
          max={line?.max}
          marked={line ? [round.answer] : undefined}
          expr={line ? undefined : `${round.promptText.replace(/\s*=\s*\?.*$/, '')} = ${fmtNum(round.answer)}`}
