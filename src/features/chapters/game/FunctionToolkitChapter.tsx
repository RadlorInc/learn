'use client'
/**
 * FunctionToolkitChapter (17–18, "Field Lab") — the Function Toolkit module.
 *
 ...
 * Flow:
 *   intro (CaseCard) → explore (TransformExplorer) → lesson → practice → done (MasteryState)
...
import TransformExplorer from '@/features/chapters/teen/sims/TransformExplorer'
import FunctionToolkitTeenLesson, { makeRound, FunctionWatch, type Round } from '@/features/chapters/lessons/FunctionToolkitTeenLesson'
...
function FunctionToolkitWorld({
      ...
      <Centered>
        <div style={{ width: '100%', maxWidth: 640 }}>
          <CaseCard
            ...
            why="Functions are input→output machines behind graphs, prices and physics — you need to read them, reshape them, and run them backward."
            question="How do we evaluate, transform, compose, and invert functions?"
            startLabel="Open the toolkit"
  ...
  if (phase === 'explore') {
    return (
      <ExploreStep
        band={BAND}
        title="Reshape a figure"
        intro="Slide the controls and watch the transformation rule update with the graph. Get a feel for how shifts, reflections and scaling move a shape, then continue."
  ...
  if (phase === 'lesson') {
    return <FunctionToolkitTeenLesson band={BAND} childName={childName} onLessonComplete={() => setPhase('practice')} />
  }
  ...
  if (phase === 'done') {
    return (
      <Centered>
        <MasteryState
          band={BAND}
          conceptsConfirmed={['Function notation & evaluation', 'Domain & range', 'Transformations', 'Composition & inverse']}
          nextPointer="Next: analyzing quadratics."
...
function FunctionToolkitPractice({
  ...
  return (
    <div className="milo-lesson" style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', background: 'var(--bg-page)', color: 'var(--ink)', fontFamily: 'var(--font-body)' }}>
      <div style={{ width: '100%', maxWidth: 640 }}>
        <TeenTopbar band={BAND} title="Function Toolkit" roundIdx={roundIdx} totalRounds={TOTAL_ROUNDS} onBack={() => { stopSpeech(); onExit() }} />
      </div>
  ...
  return createPortal(
    <div data-band={BAND} role="dialog" aria-modal="true" style={{ position: 'fixed', inset: 0, zIndex: 950, background: 'color-mix(in srgb, var(--ink) 30%, transparent)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: 'var(--paper)', border: '1px solid var(--outline)', borderRadius: 16, padding: '22px 22px 20px', maxWidth: 520, width: '100%', boxShadow: '0 6px 28px color-mix(in srgb, var(--ink) 18%, transparent)' }}>
        ...
        <FunctionWatch lines={[round.explain]} curve={round.curve} onDone={() => setReady(true)} />
...
export default function FunctionToolkitChapter(_props: Props) {
  ...
  const finish = useCallback((c: number, w: number, mastered?: boolean) => {
    ...
    finishAndSync('functionToolkit', c, w, 'practice', mastered)
  }, [finishAndSync])
