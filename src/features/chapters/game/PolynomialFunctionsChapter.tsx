'use client'
/**
 * PolynomialFunctionsChapter (17–18, "Field Lab") — the Polynomial Functions module.
 *
 * Portal pattern (like IntegersChapter): renders full-screen over the game stage,
 * sets data-band="17-18" to scope the teen theme, and calls finishAndSync itself.
 * Flow:
 *   intro (CaseCard) → explore (PolynomialExplorer) → lesson → practice → done (MasteryState)
...
import PolynomialFunctionsTeenLesson, { makeRound, PolynomialWatch, type Round } from '@/features/chapters/lessons/PolynomialFunctionsTeenLesson'
...
// ── Lightweight inline explore sim: plot a cubic/quartic; a slider flips the
// leading-coefficient sign so the end behavior flips. Exploratory, not a tool.
...
function PolynomialFunctionsWorld({
      ...
      <Centered>
        <div style={{ width: '100%', maxWidth: 640 }}>
          <CaseCard
            band={BAND}
            title="The Long View"
            why="Cost curves, trajectories and signal shapes are polynomials — their degree and leading term decide how they behave far out."
            question="How do degree, end behavior and zeros shape a polynomial?"
            startLabel="Trace the curve"
  ...
  if (phase === 'explore') {
    return (
      <ExploreStep
        band={BAND}
        title="Trace the ends"
        intro="Flip the leading sign and switch between odd and even degree. Watch how the ends of the curve swing to match — then continue."
  ...
  if (phase === 'lesson') {
    return <PolynomialFunctionsTeenLesson band={BAND} childName={childName} onLessonComplete={() => setPhase('practice')} />
  }
  ...
  if (phase === 'done') {
    return (
      <Centered>
        <MasteryState
          band={BAND}
          conceptsConfirmed={['Degree & leading term', 'End behavior', 'Real zeros & multiplicity', 'Turning points']}
...
function PolynomialFunctionsPractice({
  ...
  return (
    <div className="milo-lesson" style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', background: 'var(--bg-page)', color: 'var(--ink)', fontFamily: 'var(--font-body)' }}>
      <div style={{ width: '100%', maxWidth: 640 }}>
        <TeenTopbar band={BAND} title="Polynomial Functions" roundIdx={roundIdx} totalRounds={TOTAL_ROUNDS} onBack={() => { stopSpeech(); onExit() }} />
      </div>
  ...
  return createPortal(
    <div data-band={BAND} role="dialog" aria-modal="true" style={{ position: 'fixed', inset: 0, zIndex: 950, background: 'color-mix(in srgb, var(--ink) 30%, transparent)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: 'var(--paper)', border: '1px solid var(--outline)', borderRadius: 16, padding: '22px 22px 20px', maxWidth: 520, width: '100%', boxShadow: '0 6px 28px color-mix(in srgb, var(--ink) 18%, transparent)' }}>
        ...
        <PolynomialWatch lines={[round.explain]} curve={round.curve} onDone={() => setReady(true)} />
...
export default function PolynomialFunctionsChapter(_props: Props) {
  ...
  const finish = useCallback((c: number, w: number, mastered?: boolean) => {
    ...
    finishAndSync('polynomialFunctions', c, w, 'practice', mastered)
  }, [finishAndSync])
