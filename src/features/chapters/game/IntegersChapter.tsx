'use client'
/**
 * IntegersChapter (12–14, "Field Lab") — pilot teen chapter.
 *
 ...
 *   intro (CaseCard) → lesson (IntegersTeenLesson) → practice → done (MasteryState)
...
import NumberLineExplorer from '@/features/chapters/teen/sims/NumberLineExplorer'
import IntegersTeenLesson, { makeRound, IntegerWatch, type Round } from '@/features/chapters/lessons/IntegersTeenLesson'
      ...
      <Centered>
        <div style={{ width: '100%', maxWidth: 520 }}>
          <CaseCard
            band={BAND}
            title="Below Zero"
            why="Temperatures, depths, scores and debts all dip below zero — you need numbers that go both ways."
            question="How do negative numbers sit, compare, and measure on the number line?"
  ...
  if (phase === 'explore') {
    return (
      <ExploreStep
        ...
        intro="Slide the point along the line and watch its distance from zero and its opposite change together. Get a feel for it, then continue."
        onContinue={() => setPhase('lesson')}
      >
        <NumberLineExplorer band={BAND} />
      </ExploreStep>
    )
  }

  if (phase === 'lesson') {
    return <IntegersTeenLesson band={BAND} childName={childName} onLessonComplete={() => setPhase('practice')} />
  }
  ...
  if (phase === 'done') {
    return (
      <Centered>
        <MasteryState
          band={BAND}
          conceptsConfirmed={['Negatives on the line', 'Comparing integers', 'Absolute value', 'Distance from zero']}
...
function IntegersPractice({
  ...
  function pick(v: string | number) {
    ...
    else { setWrong((w) => w + 1); speak(`The answer is ${round.answer < 0 ? `negative ${Math.abs(round.answer)}` : round.answer}. ${ada.encouragement}`) }
  ...
  return (
    <div className="milo-lesson" style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', background: 'var(--bg-page)', color: 'var(--ink)', fontFamily: 'var(--font-body)' }}>
      <div style={{ width: '100%', maxWidth: 640 }}>
        <TeenTopbar band={BAND} title="Integers & the Number Line" roundIdx={roundIdx} totalRounds={TOTAL_ROUNDS} onBack={() => { stopSpeech(); onExit() }} />
      </div>
...
// concept on a number line, then continues (no penalty, no red).
function ReteachPanel({ round, onContinue }: { round: Round; onContinue: () => void }) {
  const [ready, setReady] = useState(false)
  if (typeof document === 'undefined') return null
  const line = round.line ?? { min: -10, max: 10, marked: [round.answer] }
  return createPortal(
    <div data-band={BAND} role="dialog" aria-modal="true" style={{ position: 'fixed', inset: 0, zIndex: 950, background: 'color-mix(in srgb, var(--ink) 30%, transparent)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: 'var(--paper)', border: '1px solid var(--outline)', borderRadius: 16, padding: '22px 22px 20px', maxWidth: 520, width: '100%', boxShadow: '0 6px 28px color-mix(in srgb, var(--ink) 18%, transparent)' }}>
        ...
        <IntegerWatch lines={[round.explain]} min={line.min} max={line.max} marked={[round.answer]} onDone={() => setReady(true)} />
