'use client'
/**
 * RatioProportionChapter (12–14, "Field Lab") — Ratios, Rates & Proportions.
 *
 * Portal pattern (like the 3–5 story chapters): renders full-screen over the
 * game stage so it bypasses the kids' zoom/celebration, sets data-band="12-14"
 * to scope the teen theme, and calls finishAndSync itself. Flow:
 *   intro (CaseCard) → lesson (RatioProportionTeenLesson) → practice → done (MasteryState)
 ...
 *
 * Rounds mix two input primitives per the interaction spec:
 *   NumericEntry for equivalent-ratio fills, unit rates, and proportion-solves;
 *   ChoiceGrid for "simplest form" and "best buy" picks.
 */
...
import RatioExplorer from '@/features/chapters/teen/sims/RatioExplorer'
...
import RatioProportionTeenLesson, { makeRound, RatioWatch, type Round } from '@/features/chapters/lessons/RatioProportionTeenLesson'
      ...
      <Centered>
        <div style={{ width: '100%', maxWidth: 520 }}>
          <CaseCard
            ...
            why="Recipes, maps, mixtures and shop shelves all hide ratios — knowing rates tells you what’s actually the better deal."
            question="How do you simplify a ratio, find a unit rate, and scale it up to solve a proportion?"
  ...
  if (phase === 'explore') {
    return (
      <ExploreStep
        band={BAND}
        title="Scale a ratio"
        intro="Set a base ratio, then slide the scale factor and watch both rows grow by the same amount — the ratio stays the same."
        onContinue={() => setPhase('lesson')}
      >
        <RatioExplorer band={BAND} />
      </ExploreStep>
    )
  }

  if (phase === 'lesson') {
    return <RatioProportionTeenLesson band={BAND} childName={childName} onLessonComplete={() => setPhase('practice')} />
  }
  ...
  if (phase === 'done') {
    return (
      <Centered>
        <MasteryState
          band={BAND}
          conceptsConfirmed={['Simplifying ratios', 'Equivalent ratios', 'Unit rates & best buy', 'Solving proportions']}
          nextPointer="Next: percentages."
...
function RatioProportionPractice({
  ...
  // shared grading tail for both primitives
  ...
  function pickChoice(v: string | number) {
    ...
    setSelected(v)
    const ok = v === round.answerValue
    grade(ok, `The better answer is shown.`)
  }

  function submitNumeric(value: number) {
    if (status !== 'idle') return
    setSelected(value)
    const ok = numericEqual(value, round.answer)
  ...
  return (
    <div className="milo-lesson" style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', background: 'var(--bg-page)', color: 'var(--ink)', fontFamily: 'var(--font-body)' }}>
      <div style={{ width: '100%', maxWidth: 640 }}>
        <TeenTopbar band={BAND} title="Ratios, Rates & Proportions" roundIdx={roundIdx} totalRounds={TOTAL_ROUNDS} onBack={() => { stopSpeech(); onExit() }} />
      </div>
        ...
        <div style={{ width: '100%', maxWidth: 380 }}>
          {round.kind === 'choice' && round.choices ? (
            <ChoiceGrid
              ...
              selected={selected}
              status={status}
              correctValue={round.answerValue}
              onPick={pickChoice}
              columns={round.columns ?? (round.choices.length === 4 ? 2 : round.choices.length)}
            />
          ) : (
            <NumericEntry
              key={roundIdx}
  ...
  return createPortal(
    <div data-band={BAND} role="dialog" aria-modal="true" style={{ position: 'fixed', inset: 0, zIndex: 950, background: 'color-mix(in srgb, var(--ink) 30%, transparent)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: 'var(--paper)', border: '1px solid var(--outline)', borderRadius: 16, padding: '22px 22px 20px', maxWidth: 520, width: '100%', boxShadow: '0 6px 28px color-mix(in srgb, var(--ink) 18%, transparent)' }}>
        ...
        {/* `big` is the mono formula plate — only show it for short, formula-style
            prompts; long word problems would overflow the numeric display. */}
        <RatioWatch big={round.promptText.length <= 28 ? round.promptText : undefined} lines={[round.explain]} onDone={() => setReady(true)} />
...
export default function RatioProportionChapter(_props: Props) {
  ...
  const finish = useCallback((c: number, w: number, mastered?: boolean) => {
    ...
    finishAndSync('ratioProportion', c, w, 'practice', mastered)
  }, [finishAndSync])
