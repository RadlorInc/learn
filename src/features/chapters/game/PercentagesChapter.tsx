'use client'
/**
 * PercentagesChapter (12–14, "Field Lab").
 *
 ...
 *   intro (CaseCard) → lesson (PercentagesTeenLesson) → practice → done (MasteryState)
 ...
 *
 * Mixes input primitives by round: L1 conversions use ChoiceGrid; L2/L3
 * (percent-of / find-the-percent / increase / decrease / reverse) use NumericEntry.
 */
...
import PercentBarExplorer from '@/features/chapters/teen/sims/PercentBarExplorer'
import PercentagesTeenLesson, { makeRound, PercentWatch, tidy, type Round } from '@/features/chapters/lessons/PercentagesTeenLesson'
...
function PercentagesWorld({
      ...
      <Centered>
        <div style={{ width: '100%', maxWidth: 520 }}>
          <CaseCard
            band={BAND}
            title="The Sale Tag"
            why="Discounts, tips, tax, interest and stats are all percentages — read them right and you never overpay or get fooled by a fake deal."
            question="How do you convert percents and find a percent of a price, an increase, or a discount?"
  ...
  if (phase === 'explore') {
    return (
      <ExploreStep
        band={BAND}
        title="Drag the percent"
        intro="Slide the percent and the total, and watch the bar fill to that share so you can feel what a percent of an amount really is."
        onContinue={() => setPhase('lesson')}
      >
        <PercentBarExplorer band={BAND} />
      </ExploreStep>
    )
  }

  if (phase === 'lesson') {
    return <PercentagesTeenLesson band={BAND} childName={childName} onLessonComplete={() => setPhase('practice')} />
  }
  ...
  if (phase === 'done') {
    return (
      <Centered>
        <MasteryState
          band={BAND}
          conceptsConfirmed={['Percent ↔ fraction ↔ decimal', 'Percent of a quantity', 'Finding the percent', 'Increase, discount & reverse']}
...
function PercentagesPractice({
  ...
  // Shared grading: ok flag + the value we'll show as "selected".
  ...
  function submitNumeric(value: number) {
    grade(numericEqual(tidy(value), round.answer, 1e-6), value)
  }
  ...
  return (
    <div className="milo-lesson" style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', background: 'var(--bg-page)', color: 'var(--ink)', fontFamily: 'var(--font-body)' }}>
      <div style={{ width: '100%', maxWidth: 640 }}>
        <TeenTopbar band={BAND} title="Percentages" roundIdx={roundIdx} totalRounds={TOTAL_ROUNDS} onBack={() => { stopSpeech(); onExit() }} />
      </div>
        ...
        {round.mode === 'choice' ? (
          ...
          <div style={{ width: '100%', maxWidth: 320 }}>
            <NumericEntry
              ...
              status={status}
              suffix={round.suffix}
              placeholder={round.suffix === '%' ? 'percent' : 'answer'}
              allowNegative={false}
            />
          </div>
        )}
...
// Spoken answer for a wrong reveal (handles the % suffix + tidy decimals).
function answerSpeech(r: Round): string {
  const v = r.answer
  if (r.suffix === '%') return `${v} percent`
  ...
  return createPortal(
    <div data-band={BAND} role="dialog" aria-modal="true" style={{ position: 'fixed', inset: 0, zIndex: 950, background: 'color-mix(in srgb, var(--ink) 30%, transparent)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: 'var(--paper)', border: '1px solid var(--outline)', borderRadius: 16, padding: '22px 22px 20px', maxWidth: 520, width: '100%', boxShadow: '0 6px 28px color-mix(in srgb, var(--ink) 18%, transparent)' }}>
        ...
        <PercentWatch lines={round.watch} onDone={() => setReady(true)} />
