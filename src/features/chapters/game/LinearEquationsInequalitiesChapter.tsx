'use client'
/**
 * LinearEquationsInequalitiesChapter (15–16) — "Saving Goal".
 *
 ...
 * reused from the lesson's ramp (see SavingGoal.tsx). data-band="15-16" scopes the
 * studio skin.
 */
...
import BalanceExplorer from '@/features/chapters/teen/sims/BalanceExplorer'
import SavingGoal from '@/features/chapters/teen/games/SavingGoal'

const BAND: AgeBand = '15-16'

type Props = { onComplete: (correct: number, wrong: number) => void; childName: string }

function LinearEquationsInequalitiesWorld({
  ...
  if (phase === 'explore') {
    return (
      <ExploreStep
        ...
        intro="Slide x until the beam sits level. Whatever keeps both pans equal is the solution — same idea as the game. Have a play, or skip straight to the savings plan."
...
export default function LinearEquationsInequalitiesChapter(_props: Props) {
  ...
  const finish = useCallback((c: number, w: number, mastered?: boolean) => {
    ...
    finishAndSync('linearEquationsInequalities', c, w, 'practice', mastered)
  }, [finishAndSync])
  ...
  return createPortal(
    <div data-band={BAND} style={{ position: 'fixed', inset: 0, zIndex: 900, overflowY: 'auto', background: 'var(--bg-page)', color: 'var(--ink)' }}>
      <LinearEquationsInequalitiesWorld
        key={runKey}
