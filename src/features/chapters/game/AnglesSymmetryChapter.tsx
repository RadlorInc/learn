'use client'
/**
 * AnglesSymmetryChapter (9–11) — name an angle (acute/right/obtuse) and count lines of
 * symmetry — as the AngleScope pre-teen "Number Lab" experience (see story/AngleScope.tsx
 ...
 * Reuses skill `anglesSymmetry`.
 */
import { createPortal } from 'react-dom'
import { useEffect, useState, useRef, useCallback } from 'react'
import { useRouter } from 'next/navigation'
import AngleScope from '@/features/chapters/story/AngleScope'
import { useChapterSync } from '@/data/supabase/useChapterSync'
import CelebrationModal from '@/shared/ui/CelebrationModal'

export default function AnglesSymmetryChapter(_props: { onComplete: (correct: number, wrong: number) => void; childName: string }) {
  ...
  const finish = useCallback((correct: number, wrong: number, mastered?: boolean) => {
    ...
    finishAndSync('anglesSymmetry', correct, wrong, 'practice', mastered)
  }, [finishAndSync])
  ...
  return createPortal(
    <div style={{ position: 'fixed', inset: 0, zIndex: 900, background: '#0a1026' }}>
      <AngleScope key={runKey} onFinish={finish} onExit={() => router.push('/menu')} />
