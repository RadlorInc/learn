'use client'
/**
 * CoordinatePlaneChapter (12–14) — PLAYABLE-GAME chapter shape (no slides, no MCQ).
 * One continuous NightFlight scene; Milo demos order #1 in-scene, then the kid plays.
  ...
  if (phase === 'done') {
    return (
      <div className="milo-lesson" style={{ minHeight: '100dvh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px 18px', background: 'var(--bg-page)', color: 'var(--ink)', fontFamily: 'var(--font-body)', boxSizing: 'border-box' }}>
        <MasteryState
          band={BAND}
          conceptsConfirmed={['Plotting (x, y)', 'All four quadrants', 'Reflecting across an axis', 'Finding a midpoint']}
          nextPointer="Next: linear relationships."
