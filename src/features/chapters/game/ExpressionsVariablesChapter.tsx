'use client'
/**
 * ExpressionsVariablesChapter (15–16, "Design Studio") — teen chapter.
 *
 * Portal pattern (like IntegersChapter): renders full-screen over the game
 * stage, sets data-band="15-16" (dark studio skin) to scope the teen theme, and
 * calls finishAndSync itself. Flow:
 *   intro (CaseCard) → explore (ExpressionEvaluator) → lesson → practice → done
 ...
 * Math-without-fear: no timer, no red/X, amber-not-red, ~1600ms feedback pacing.
 */
...
import ExpressionsVariablesTeenLesson, { makeRound, ExprWatch, type Round } from '@/features/chapters/lessons/ExpressionsVariablesTeenLesson'
...
function ExpressionsVariablesWorld({
      ...
      <Centered>
        <div style={{ width: '100%', maxWidth: 520 }}>
          <CaseCard
            ...
            why="A booth charges a setup fee plus a rate per item — one expression with a variable prices any order, no recalculating from scratch."
            question="How do you build, evaluate, and simplify expressions with variables?"
  ...
  if (phase === 'explore') {
    return (
      <ExploreStep
        band={BAND}
        title="Slide x"
        intro="Slide x and watch the expression resolve to a value — the same x is a point on the line. Get a feel for it, then continue."
  ...
  if (phase === 'lesson') {
    return <ExpressionsVariablesTeenLesson band={BAND} childName={childName} onLessonComplete={() => setPhase('practice')} />
  }
  ...
  return (
    <div className="milo-lesson" style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', background: 'var(--bg-page)', color: 'var(--ink)', fontFamily: 'var(--font-body)' }}>
      <div style={{ width: '100%', maxWidth: 640 }}>
        <TeenTopbar band={BAND} title="Expressions & Variables" roundIdx={roundIdx} totalRounds={TOTAL_ROUNDS} onBack={() => { stopSpeech(); onExit() }} />
      </div>
...
// concept with a worked example, then continues (no penalty, no red).
  ...
  return createPortal(
    <div data-band={BAND} role="dialog" aria-modal="true" style={{ position: 'fixed', inset: 0, zIndex: 950, background: 'color-mix(in srgb, var(--ink) 30%, transparent)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: 'var(--paper)', border: '1px solid var(--outline)', borderRadius: 16, padding: '22px 22px 20px', maxWidth: 520, width: '100%', boxShadow: '0 6px 28px color-mix(in srgb, var(--ink) 18%, transparent)' }}>
        ...
        <ExprWatch lines={[round.explain]} display={round.promptText} result={String(round.answer)} onDone={() => setReady(true)} />
