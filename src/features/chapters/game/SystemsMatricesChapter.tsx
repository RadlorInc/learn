'use client'
/**
 * SystemsMatricesChapter (17-18, "Field Lab") — "Where Lines Meet".
 *
 ...
 * sets data-band="17-18" to scope the teen theme, and calls finishAndSync
 * itself. Flow:
 *   intro (CaseCard) → explore (ExploreStep + SystemExplorer) → lesson →
 *   practice → done (MasteryState)
...
import SystemsMatricesTeenLesson, { makeRound, SystemsWatch, type Round } from '@/features/chapters/lessons/SystemsMatricesTeenLesson'
...
function SystemsMatricesWorld({
    ...
    return (
      <Centered>
        <div style={{ width: '100%', maxWidth: 640 }}>
          <CaseCard
            band={BAND}
            title="Where Lines Meet"
            why="Two conditions at once — prices, mixtures, forces — are solved where the lines cross; matrices scale that up to many unknowns."
            question="How do we solve systems, and compute with matrices?"
            startLabel="Solve the system"
  ...
  if (phase === 'explore') {
    return (
      <ExploreStep
        ...
        intro="Slide the slopes and intercept and watch where the two lines meet. Equal slopes never cross. Get a feel for it, then continue."
  ...
  if (phase === 'lesson') {
    return <SystemsMatricesTeenLesson band={BAND} childName={childName} onLessonComplete={() => setPhase('practice')} />
  }
  ...
  if (phase === 'done') {
    return (
      <Centered>
        <MasteryState
          band={BAND}
          conceptsConfirmed={['Solving 2×2 systems', 'One / none / infinite solutions', 'Matrix add & scalar multiply', '2×2 determinant']}
          nextPointer="Next: sequences & series."
...
function SystemsMatricesPractice({
  ...
  return (
    <div className="milo-lesson" style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', background: 'var(--bg-page)', color: 'var(--ink)', fontFamily: 'var(--font-body)' }}>
      <div style={{ width: '100%', maxWidth: 640 }}>
        <TeenTopbar band={BAND} title="Systems & Matrices" roundIdx={roundIdx} totalRounds={TOTAL_ROUNDS} onBack={() => { stopSpeech(); onExit() }} />
      </div>
...
// missed concept with speakSeq, then continues (no penalty, no red).
  ...
  return createPortal(
    <div data-band={BAND} role="dialog" aria-modal="true" style={{ position: 'fixed', inset: 0, zIndex: 950, background: 'color-mix(in srgb, var(--ink) 30%, transparent)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: 'var(--paper)', border: '1px solid var(--outline)', borderRadius: 16, padding: '22px 22px 20px', maxWidth: 520, width: '100%', boxShadow: '0 6px 28px color-mix(in srgb, var(--ink) 18%, transparent)' }}>
        ...
        <SystemsWatch lines={[round.explain]} onDone={() => setReady(true)} />
...
export default function SystemsMatricesChapter(_props: Props) {
  ...
  const finish = useCallback((c: number, w: number, mastered?: boolean) => {
    ...
    finishAndSync('systemsMatrices', c, w, 'practice', mastered)
  }, [finishAndSync])
