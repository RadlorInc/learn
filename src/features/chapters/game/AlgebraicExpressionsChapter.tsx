'use client'
/**
 * AlgebraicExpressionsChapter (12–14, "Field Lab").
 *
 ...
 *   intro (CaseCard) → lesson (AlgebraicExpressionsTeenLesson) → practice → done
 ...
 *
 * Rounds mix two input primitives per their mode: NumericEntry (evaluate at a
 * value) and ChoiceGrid (combine like terms / distribute / simplify — tap the
...
import AlgebraicExpressionsTeenLesson, { makeRound, ExprWatch, fmtInt, type Round } from '@/features/chapters/lessons/AlgebraicExpressionsTeenLesson'
...
function AlgebraicExpressionsWorld({
    ...
    return (
      <Centered>
        <div style={{ width: '100%', maxWidth: 520 }}>
          <CaseCard
            band={BAND}
            title="The Rental Formula"
            why="A skate shop charges a flat fee plus an hourly rate. One expression — x hours — prices every visit, so you stop redoing the maths each time."
            question="How do you write, simplify, and evaluate an expression that stands in for many numbers?"
  ...
  if (phase === 'explore') {
    return (
      <ExploreStep
        band={BAND}
        title="Drag x"
        intro="The expression 3x + 2 stands in for many numbers — drag x and watch the substitution resolve and its point trace the line."
  ...
  if (phase === 'lesson') {
    return <AlgebraicExpressionsTeenLesson band={BAND} childName={childName} onLessonComplete={() => setPhase('practice')} />
  }
  ...
  if (phase === 'done') {
    return (
      <Centered>
        <MasteryState
          band={BAND}
          conceptsConfirmed={['Evaluating at a value', 'Combining like terms', 'Distributing over brackets', 'Multi-variable simplifying']}
...
function AlgebraicExpressionsPractice({
  ...
  // Shared grading tail for either input primitive.
  ...
  function pickChoice(v: string | number) {
    ...
    setSelected(v)
    const ok = v === round.choiceAnswer
    grade(ok, `The answer is ${round.choiceAnswer}.`)
  }

  function submitNumeric(v: number) {
    if (status !== 'idle') return
    setSelected(v)
    const ok = numericEqual(v, round.answer)
    grade(ok, `The answer is ${round.answer < 0 ? `negative ${Math.abs(round.answer)}` : round.answer}.`)
  }
  ...
  return (
    <div className="milo-lesson" style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', background: 'var(--bg-page)', color: 'var(--ink)', fontFamily: 'var(--font-body)' }}>
      <div style={{ width: '100%', maxWidth: 640 }}>
        <TeenTopbar band={BAND} title="Algebraic Expressions" roundIdx={roundIdx} totalRounds={TOTAL_ROUNDS} onBack={() => { stopSpeech(); onExit() }} />
      </div>
      ...
      <main style={{ flex: 1, width: '100%', maxWidth: 560, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 20, padding: '12px 18px 28px', boxSizing: 'border-box' }}>
        ...
        {round.mode === 'choice' && round.choices ? (
          <div style={{ width: '100%', maxWidth: 440 }}>
            <ChoiceGrid
              ...
              selected={selected}
              status={status}
              correctValue={round.choiceAnswer}
              onPick={pickChoice}
              columns={2}
            />
          </div>
...
function ReteachPanel({ round, onContinue }: { round: Round; onContinue: () => void }) {
  ...
  const headline = round.mode === 'numeric'
    ? (round.answer < 0 ? fmtInt(round.answer) : String(round.answer))
    : round.choiceAnswer
  return createPortal(
    <div data-band={BAND} role="dialog" aria-modal="true" style={{ position: 'fixed', inset: 0, zIndex: 950, background: 'color-mix(in srgb, var(--ink) 30%, transparent)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: 'var(--paper)', border: '1px solid var(--outline)', borderRadius: 16, padding: '22px 22px 20px', maxWidth: 520, width: '100%', boxShadow: '0 6px 28px color-mix(in srgb, var(--ink) 18%, transparent)' }}>
        ...
        <ExprWatch headline={headline} lines={[round.explain]} onDone={() => setReady(true)} />
