'use client'
/**
 * LinearRelationshipsChapter (12–14, "Field Lab").
 *
 * Portal pattern (like the 3–5 story chapters + IntegersChapter): renders
 * full-screen over the game stage so it bypasses the kids' zoom/celebration,
 * sets data-band="12-14" to scope the teen theme, and calls finishAndSync
 * itself. Flow:
 *   intro (CaseCard) → lesson (LinearRelationshipsTeenLesson) → practice → done
 * Uses the SAME engine as every chapter: useAdaptive (L1/L2/L3). The practice
 * screen mixes answer surfaces by difficulty: NumericEntry (table value / slope)
 * and ChoiceGrid (match equation ↔ line), each over a rendered CoordGrid.
...
import LinearRelationshipsTeenLesson, {
  makeRound, LineWatch, ValueTable, spoken, type Round,
} from '@/features/chapters/lessons/LinearRelationshipsTeenLesson'
...
function LinearWorld({
    ...
    return (
      <Centered>
        <div style={{ width: '100%', maxWidth: 520 }}>
          <CaseCard
            band={BAND}
            title="The Phone-Plan Showdown"
            why="Pay-as-you-go, data caps, ride fares — most real costs grow at a steady rate. A linear rule lets you predict and compare them before you commit."
            question="How do a rule, a table, and a line all describe the same steady relationship?"
  ...
  if (phase === 'explore') {
    return (
      <ExploreStep
        ...
        intro="Move the sliders and watch the line, the equation, and the rise/run change together. Get a feel for it, then continue."
  ...
  if (phase === 'lesson') {
    return <LinearRelationshipsTeenLesson band={BAND} childName={childName} onLessonComplete={() => setPhase('practice')} />
  }
  ...
  if (phase === 'done') {
    return (
      <Centered>
        <MasteryState
          band={BAND}
          conceptsConfirmed={['Tables from a rule', 'Slope / rate of change', 'Reading y = mx + b', 'Matching equation to line']}
          nextPointer="Next: area, volume & the Pythagorean theorem."
...
function LinearPractice({
  ...
  const ada = useAdaptive('linearRelationships')
  ...
  // Spoken form of the revealed answer (numeric or equation choice).
  function answerSpeech(r: Round): string {
    if (r.input === 'numeric' && r.answerNum != null) return `The answer is ${spoken(r.answerNum)}.`
    if (r.input === 'choice' && r.answerChoice != null) return `The answer is ${r.answerChoice}.`
    return 'Here is the answer.'
  }

  function grade(ok: boolean, shownValue: string | number) {
    if (status !== 'idle') return
    setSelected(shownValue)
  ...
  function pick(v: string | number) {
    grade(v === round.answerChoice, v)
  }
  // NumericEntry submit.
  function submitNum(v: number) {
    grade(round.answerNum != null && numericEqual(v, round.answerNum), v)
  }
  ...
  return (
    <div className="milo-lesson" style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', background: 'var(--bg-page)', color: 'var(--ink)', fontFamily: 'var(--font-body)' }}>
      <div style={{ width: '100%', maxWidth: 640 }}>
        <TeenTopbar band={BAND} title="Linear Relationships & Functions" roundIdx={roundIdx} totalRounds={TOTAL_ROUNDS} onBack={() => { stopSpeech(); onExit() }} />
      </div>
      ...
      <main style={{ flex: 1, width: '100%', maxWidth: 560, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 18, padding: '12px 18px 28px', boxSizing: 'border-box' }}>
        ...
        {/* The graph is shown for graph-based rounds (slope from line, match eqn). */}
        {(round.input === 'choice' || round.input === 'numeric' && !round.table) && (
          <div style={{ width: '100%', maxWidth: 320 }}>
            <CoordGrid band={BAND} xRange={[-6, 6]} yRange={[-6, 6]} mode="read" lines={[round.line]} />
          </div>
        )}

        {/* The value table is shown for table rounds (L1). */}
        {round.table && <ValueTable xs={round.table.xs} ys={round.table.ys} />}

        {round.input === 'numeric' && (
          <NumericEntry
            band={BAND}
            status={status}
            onSubmit={(v) => submitNum(v)}
            placeholder={round.table ? 'y' : 'slope'}
            allowDecimal={false}
          />
        )}

        {round.input === 'choice' && round.choices && (
          <div style={{ width: '100%', maxWidth: 400 }}>
            <ChoiceGrid
              ...
              selected={selected}
              status={status}
              correctValue={round.answerChoice}
              onPick={pick}
