...
export const G7M5: Lesson[] = [
  // ── Topic 1 ──────────────────────────────────────────────────────────────────────────────────
  {
    ...
    screens: [
      { title: 'The favorite sport at school', text: 'A school has 400 students. You want to know their favorite sport, but you can only ask 20 of them. Which 20 should you ask?',
        pictures: [SCHOOL] },
      { title: 'Asking the basketball team', text: 'The 20 kids on the basketball team are easy to find. But almost all of them say basketball. Their answers do not look like the whole school.',
        pictures: [TEAM] },
      { title: 'The big idea', text: 'A fair sample gives everyone in the group the same chance to be picked. Asking only one kind of person is not fair.',
        pictures: [SCHOOL] },
      { title: 'Pick names from a hat', text: 'Put all 400 names in a hat. Pull out 20 without looking. Every student has the same chance to be picked.',
        pictures: [{ ...HAT, motion: true }] },
      { title: 'It looks like the school', text: '7 of the 20 names from the hat picked soccer. That is 35 out of 100. In the whole school, 140 of 400 picked soccer. That is also 35 out of 100.',
        pictures: [HAT, { kind: 'eq', text: '7/20 = 35/100', lines: ['140/400 = 35/100'] }] },
      { title: 'Ask who is left out', text: 'Could anyone in the school never be picked? Asking the team leaves out every kid who is not on it. The hat leaves out nobody.',
        pictures: [{ ...TEAM, motion: true }, HAT] },
      { title: 'One thing not to do', text: "Don't call a sample fair just because it is big. 100 fans at a basketball game still mostly pick basketball.",
  ...
  // ── Topic 2 ──────────────────────────────────────────────────────────────────────────────────
  {
    ...
    screens: [
      { title: 'Pizza for the whole school', text: 'A school has 600 students. You ask 40 of them, picked by chance. 12 say pizza is their favorite lunch. How many of all 600 like pizza best?',
        pictures: [PIZZA_TAPE] },
      { title: 'It is not just 12', text: '12 is only the kids you asked. The school is much bigger than 40, so many more kids like pizza best.',
        pictures: [PIZZA_TAPE] },
      { title: 'The big idea', text: 'A fair sample is a small copy of the whole group. Find its fraction, then take that fraction of the whole group.',
        pictures: [PIZZA_TAPE] },
      { title: 'Find the fraction', text: '12 out of 40 like pizza. 12/40 = 3/10. So about 3 out of every 10 kids like pizza best.',
        pictures: [{ kind: 'tape', rows: [{ label: 'Every 10 kids', cells: tens() }], motion: true }, { kind: 'eq', text: '12/40 = 3/10' }] },
      { title: 'Stretch it to 600', text: 'The whole school is the same tape, just bigger. Split 600 into 10 equal parts: 600 ÷ 10 = 60 kids in each part.',
        pictures: [{ kind: 'tape', rows: [{ label: 'School', cells: tens('60'), brace: '600 kids' }], motion: true }, { kind: 'eq', text: '600 ÷ 10 = 60' }] },
      { title: 'Count the pizza parts', text: '3 of the parts like pizza. 3 × 60 = 180. So predict that 180 of the 600 students like pizza best.',
        pictures: [{ kind: 'tape', rows: [{ label: 'School', cells: tens('60'), brace: '600 kids' }] }, { kind: 'eq', text: '3/10 of 600', lines: ['3 × 60 = 180'] }] },
      { title: 'One thing not to do', text: "Don't stop at the count in the sample. 12 is for the 40 kids you asked. Scale it up to the whole group.",
  ...
  // ── Topic 3 ──────────────────────────────────────────────────────────────────────────────────
  {
    ...
    screens: [
      ...
      { title: 'One plant does not tell you', text: "Class B's tallest plant is 7 inches, and Class A has a plant that is only 5. Pick single plants and you can show anything. We need one number for each whole class.",
        pictures: [CLASS_A, CLASS_B] },
      { title: 'The big idea', text: 'To compare two groups, find the gap between their means. Then look at the spread to see if that gap is big or small.',
        pictures: [CLASS_A, CLASS_B] },
      { title: 'Find each mean', text: 'Class A: 5 + 6 + 6 + 7 + 7 + 7 + 8 + 8 + 9 = 63, and 63 ÷ 9 = 7. Class B adds to 45, and 45 ÷ 9 = 5.',
        pictures: [{ ...CLASS_A, motion: true }, { ...CLASS_B, motion: true }, { kind: 'eq', text: '63 ÷ 9 = 7', lines: ['45 ÷ 9 = 5'] }] },
      { title: 'Find the gap', text: '7 − 5 = 2. On average, Class A grew plants 2 inches taller.',
        pictures: [CLASS_A, CLASS_B, { kind: 'eq', text: '7 − 5 = 2' }] },
      { title: 'Is the gap big?', text: 'Each class spreads over 4 inches: 9 − 5 = 4 and 7 − 3 = 4. The gap of 2 is only half of that, so the two plots overlap a lot. Class A is a bit taller, not a lot.',
        pictures: [CLASS_A, CLASS_B, { kind: 'eq', text: '9 − 5 = 4', lines: ['7 − 3 = 4'] }] },
      { title: 'One thing not to do', text: "Don't compare only the tallest plants. Compare the means, then check the spread.",
  ...
  // ── Topic 4 ──────────────────────────────────────────────────────────────────────────────────
  {
    ...
    screens: [
      { title: 'Spin to win', text: 'This spinner has 10 equal parts. You win if it lands on blue. What is your chance, as a fraction and as a decimal?',
        pictures: [WIN] },
      { title: 'Four colors does not mean 1/4', text: 'There are four colors, but they are not the same size. Blue covers more parts than yellow. Count parts, not colors.',
        pictures: [WIN] },
      { title: 'The big idea', text: 'A probability is the ways it can happen over all the equal ways. It is a fraction or a decimal from 0 to 1.',
        pictures: [WIN] },
      { title: 'Count the ways', text: 'Count the blue parts: 4. Count all the parts: 10. The chance of blue is 4/10.',
        pictures: [WIN, WIN_COUNTS(true), { kind: 'eq', text: '4/10' }] },
      { title: 'Write it as a decimal', text: '4/10 is four tenths. As a decimal that is 0.4. Both say the same chance.',
        pictures: [WIN, { kind: 'eq', text: '4/10 = 2/5', lines: ['= 0.4'] }] },
      { title: 'And not blue?', text: 'The other 6 parts are not blue: 6/10 = 0.6. Together 0.4 + 0.6 = 1, because the spinner always lands somewhere.',
        pictures: [WIN, { kind: 'table', head: ['blue', 'not blue'], rows: [['0.4', '0.6']], motion: true }, { kind: 'eq', text: '0.4 + 0.6 = 1' }] },
      { title: 'One thing not to do', text: "Don't put blue over the parts that are not blue. Put blue over ALL the parts.",
  ...
  // ── Topic 5 ──────────────────────────────────────────────────────────────────────────────────
  {
    ...
    screens: [
      ...
      { title: 'Guessing is not enough', text: 'One kid guessed 40 heads, another guessed 10. A guess can be anything. The chance of heads on each flip gives a better number.',
        pictures: [flips(27, 23, 5)] },
      { title: 'The big idea', text: 'Expected count = probability × number of tries. Real results land near it, but not always right on it.',
        pictures: [flips(27, 23, 5)] },
      { title: 'Find the expected count', text: 'Heads has a chance of 1/2 on each flip. 1/2 × 50 = 25. So we expect 25 heads.',
        pictures: [flips(25, 25, 5, true), { kind: 'eq', text: '1/2 × 50 = 25' }] },
      { title: 'The real results', text: 'The class got 27 heads and 23 tails. That is close to 25, but not exactly. Each flip is new, so real results wander a little.',
        pictures: [flips(27, 23, 5, true), { kind: 'eq', text: 'expected 25', lines: ['real 27'] }] },
      { title: 'More flips, closer', text: 'Another class flipped 500 times and got 246 heads. 246/500 = 0.492, very close to 1/2 = 0.5. The more tries, the closer the fraction gets.',
        pictures: [flips(246, 254, 50), { kind: 'eq', text: '27/50 = 0.54', lines: ['246/500 = 0.492'] }] },
      { title: 'One thing not to do', text: "Don't think the real count must equal the expected count. 27 heads out of 50 is not a mistake.",
  ...
  // ── Topic 6 ──────────────────────────────────────────────────────────────────────────────────
  {
    ...
    screens: [
      { title: 'Flip and roll', text: 'You flip a coin and roll a number cube. What is the chance of getting heads AND a 5?',
        pictures: [COIN_DIE] },
      { title: 'Adding the chances does not work', text: 'Heads is 1/2 and a 5 is 1/6. Adding gives 1/2 + 1/6 = 4/6, bigger than either one. Getting both at once should be harder, not easier.',
      ...
      { title: 'The big idea', text: 'To find the chance of two things together, list every pair in a table. Count the pairs you want over all the pairs.',
        pictures: [COIN_DIE] },
      { title: 'Fill in every pair', text: 'Each row is the coin. Each column is the cube. Each box is one way both can land: H1, H2, and so on to T6.',
        pictures: [COIN_DIE_FULL(undefined, true)] },
      { title: 'Count all the pairs', text: '2 rows × 6 columns = 12 boxes. So there are 12 pairs, and each one is equally likely.',
        pictures: [COIN_DIE_FULL(), { kind: 'eq', text: '2 × 6 = 12' }] },
      { title: 'Find the pair you want', text: 'Heads and a 5 is just one box: H5. So the chance is 1 out of 12, or 1/12.',
        pictures: [COIN_DIE_FULL([[0, 5]], true), { kind: 'eq', text: '1/12' }] },
      { title: 'One thing not to do', text: "Don't add the two chances. Make the table and count the pairs.",
