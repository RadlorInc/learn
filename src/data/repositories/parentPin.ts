...
export type PinResult = { ok: true; reset_cancelled?: boolean } | { ok: false; error: string; locked_until?: string | null; tries_left?: number }
...
export async function getPinStatus(): Promise<PinStatus> {
  try {
    ...
    if (error) return missing(error) ? { state: 'unavailable' } : { state: 'error' }
...
async function call(fn: string, args: Record<string, unknown>): Promise<PinResult> {
  try {
    ...
    return error ? { ok: false, error: 'failed' } : (data as PinResult)
