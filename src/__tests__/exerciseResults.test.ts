...
beforeAll(async () => {
  ...
  const learner = async (kid: string) => {
    const id = (await db.query<{ id: string }>(`insert into public.learners (display_name, avatar_index, age_group, created_by, grade_id, consent_id, attested_notice_version)
      values ('Kid', 0, '9-11', '${TEACHER}', '${CLASS}', '${await grantedConsent(db, TEACHER)}', '${FIXTURE_NOTICE}') returning id`)).rows[0].id
