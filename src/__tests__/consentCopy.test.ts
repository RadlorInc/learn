...
const bCopy = en([
  B1.subject, B1.hi, B1.before, ...B1.list, B1.doNot, B1.ignore, B1.details, B1.address, B1.covers, B1.tick, B1.decline,
  B2.heading, ...B2.body,
...
describe('Spanish — present everywhere, and never claimed to be reviewed', () => {
  const all: L[] = [
    ...
    B1.subject, B1.hi, B1.before, ...B1.list, B1.doNot, B1.ignore, B1.details, B1.covers, B1.tick, B1.decline,
...
describe('the rendered emails and screen say nothing the documents do not', () => {
  ...
  it('B1 and B3, both parts', async () => {
    ...
    const own = (l: string) => /^☐ I agree: https:/.test(l)
    for (const [name, m] of [['B1', b1], ['B3', b3]] as const) {
      expect(extra(m.text.split('\n').filter(l => !own(l))), `${name} text part`).toEqual([])
      expect(extra(htmlLines(m.html).filter(l => !/^I agree$/.test(l.trim()) && !own(l))), `${name} html part`).toEqual([])
