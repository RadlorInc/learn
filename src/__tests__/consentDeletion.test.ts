...
/** A child with a row in every table document 06 names, and their own sign-in account. */
async function seedChild(name: string, withLogin = false): Promise<{ id: string; consent: string; token: string }> {
  ...
  const [{ id }] = await q<{ id: string }>(`insert into public.learners (display_name, avatar_index, age_group, created_by, consent_id)
    values ('${name}', 0, '6-8', '${PARENT}', '${consent}') returning id`)
  await db.exec(`
...
describe('deleting a child deletes every row document 06 lists', () => {
  beforeAll(async () => {
    ({ db } = await loadSchema())
    await db.exec(`insert into auth.users (id, email, email_confirmed_at) values ('${PARENT}', 'parent@x.test', now());
      insert into public.profiles (id, role) values ('${PARENT}', 'parent') on conflict (id) do update set role = excluded.role;`)
  }, 120_000)

  it('withdrawal — rows before in every table, none after; the consent record kept as withdrawn', async () => {
    const kid = await seedChild('Withdrawn', true)
    const sibling = await seedChild('Sibling')
    const before = await countsFor(kid.id)
    for (const t of DOC06) expect(before[t], `${t} had no row for the child BEFORE — a zero after would prove nothing`).toBeGreaterThan(0)

    const [{ s }] = await q<{ s: string }>(`select public.consent_withdraw('${kid.token}') as s`)
    expect(s).toBe('withdrawn')

    const after = await countsFor(kid.id)
    expect(after).toEqual(Object.fromEntries(DOC06.map(t => [t, 0])))
    for (const t of await everyLearnerTable())
      expect(Number((await q<{ n: number }>(`select count(*)::int n from public.${t} where learner_id = '${kid.id}'`))[0].n), `${t} still holds the child`).toBe(0)
    ...
    expect(rec, 'the consent record — the evidence — was deleted with the child').toBeDefined()
    expect(rec.state).toBe('withdrawn')
    expect(rec.learner_id).toBeNull()
    expect(rec.withdrawn_at).not.toBeNull()

    // Only that child: the sibling on the same account is untouched.
    expect(Object.values(await countsFor(sibling.id)).every(n => Number(n) > 0)).toBe(true)
  })

  it('"Delete <name>\'s profile" — the same set, and the consent ended so it cannot be reused', async () => {
    const kid = await seedChild('Removed')
    const before = await countsFor(kid.id)
    for (const t of DOC06.filter(t => t !== 'learner_access' || before[t] > 0)) expect(before[t]).toBeGreaterThan(0)

    expect(await as(PARENT, `select public.delete_learner('${kid.id}')`)).toBeNull()
    ...
    expect(rec.state, 'a granted consent left with no child looks unused, and the add-a-child flow reuses unused consents').toBe('withdrawn')
  })

  it('only the owner may delete, and the refusal is the ownership check', async () => {
    const kid = await seedChild('Kept')
