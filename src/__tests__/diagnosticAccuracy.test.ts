...
/**
 * The guess rate of the REAL item, per skill — what a child who does NOT have the skill still comes
 * back right with.
 *
 * ⚠️ THIS USED TO BE A FLAT 3% FOR ANYTHING TYPED, AND THAT WAS TOO KIND. A typed answer is not
 * unguessable, it is only as unguessable as its ANSWER SPACE is wide — and measuring that turned up
 * `i.dataGraphs` shuffling a fixed `[2,4,6,9]`, so the tallest bar minus the shortest was **always
 * 7**: one possible answer across every draw the generator could make. The numbers on screen looked
 * varied because the BARS were. So the rate is derived from what the generator can actually produce,
 * which is the only thing a child could be guessing among.
 */
const GUESS: Record<string, number> = {}
for (const n of SKILL_NODES) {
  const answers = new Set<string>()
  let choices = 0, pick = false, none = false
  for (let i = 0; i < 120; i++) {
    const it = makeItem(n.id, { seed: `calib${i}`, nonce: i })
    if (!it) { none = true; break }
    if (it.input === 'pick') { pick = true; choices = Math.max(choices, it.choices.length) }
    answers.add(it.answer)
  }
  // no item at all → the engine auto-passes it, which is a certainty rather than a guess
  GUESS[n.id] = none ? 1 : pick ? 1 / Math.max(choices, 2) : Math.min(0.25, 1 / Math.max(answers.size, 4))
}
...
describe('diagnostic accuracy (simulated learners)', () => {
  ...
  it('names the EXACT planted root gap for most children', { timeout: 60000 }, () => {
    ...
    // ⚠️ AND THE TABLE IS PRINTED BEFORE ANYTHING IS ASSERTED. It used to assert inside the loop,
    //  so the first band under threshold threw and the other four numbers — the ones you need to
    //  see what actually moved — were never printed at all.
    const rows: string[] = []
    const fails: string[] = []
    for (const band of BANDS) {
      ...
      // v1 measured 26–34% here and it now measures 96–98%. Anything back under 85 means either the
      // answer surface has become guessable again (a `pick`, or a generator whose answer space has
      // narrowed — that is how `i.dataGraphs` was caught with ONE possible answer) or a confirmation
      // has been traded away for length.
      if (pct(o.exact, o.n) < 92) fails.push(`${band} exact root ${pct(o.exact, o.n)}% < 92%`)
      // ⚠️ The error that matters most, gated here as well as in its own test below: a child WITH a
      // gap must not be told they have none.
      if (pct(o.none, o.n) > 2) fails.push(`${band} missed the gap entirely ${pct(o.none, o.n)}% > 2%`)
      /**
       ...
       * probe grow without anybody deciding to let it. ⚠️ The number that actually protects the
       * anti-fear rule is the ON-GRADE one below: a child with nothing wrong must not be put
       * through an ordeal, and they are not, because the extra evidence is spent on doubt.
       */
      const cap = { '6-8': 44, '9-11': 56, '12-14': 58, '15-16': 52, '17-18': 70 }[band as string] ?? 56
      if (p95(o.asks) > cap) fails.push(`${band} probe length p95 ${p95(o.asks)} > ${cap}`)
    }
  ...
  it('almost never tells a child with a real gap that they are on track', { timeout: 30000 }, () => {
    for (const band of BANDS) {
      ...
      expect(pct(o.none, o.n), `${band} missed the gap entirely`).toBeLessThanOrEqual(2)
    }
  })
  ...
  it('rarely invents a gap for an on-grade child, and essentially never a deep one', () => {
    ...
    for (const band of BANDS) {
      ...
      expect(pct(falseGap, T), `${band} false gap (any)`).toBeLessThanOrEqual(8)
      ...
      /** ⚠️ THE ANTI-FEAR NUMBER, and it is no longer small: confirming every answer means a child
       *  with NO gap still answers 20–36. That is the price of 96–98%, it is the founder's call
       *  (2026-08-22, accuracy over length), and the intro copy was rewritten to say so — "about
       *  ten minutes" rather than the "2 minutes" it promised while this was a coin flip. */
      expect(p95(asks), `${band} on-grade length`).toBeLessThanOrEqual(PROBE_ENTRY[band].length * 3 + 12)
    }
  })
