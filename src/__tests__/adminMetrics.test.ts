...
beforeAll(async () => {
  ...
  overview = (await db.query(`select public.admin_overview(1)`)).rows[0].admin_overview
  learning = (await db.query(`select public.admin_learning(1)`)).rows[0].admin_learning
  funnel   = (await db.query(`select public.admin_funnel(1)`)).rows[0].admin_funnel
}, 90_000)
