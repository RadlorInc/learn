'use client'
/**
 * ExpLogFunctionsChapter (17–18, "Field Lab" / math-studio) — Exponential & Log.
 *
 ...
 *   intro (CaseCard) → explore (GrowthExplorer sim) → lesson → practice → done
...
import GrowthExplorer from '@/components/teen/sims/GrowthExplorer'
import ExpLogFunctionsTeenLesson, { makeRound, ExpLogWatch, type Round } from '@/components/lessons/ExpLogFunctionsTeenLesson'
...
function ExpLogFunctionsWorld({
    ...
    return (
      <Centered>
        <div style={{ width: '100%', maxWidth: 520 }}>
          <CaseCard
            band={BAND}
            title="Doubling & Half-Lives"
            why="Populations, interest, and radioactive decay all move exponentially — and logarithms are the tool that undoes them."
            question="How do exponential growth/decay and logarithms work together?"
            startLabel="Run the model"
  ...
  if (phase === 'explore') {
    return (
      <ExploreStep
        band={BAND}
        title="Race the models"
        intro="Slide the rates and watch a straight line lose to an exponential curve. Get a feel for how fast compounding takes over, then continue."
  ...
  if (phase === 'lesson') {
    return <ExpLogFunctionsTeenLesson band={BAND} childName={childName} onLessonComplete={() => setPhase('practice')} />
  }
  ...
  if (phase === 'done') {
    return (
      <Centered>
        <MasteryState
          band={BAND}
          conceptsConfirmed={['Growth vs decay', 'Evaluating exponentials', 'Logs as inverses', 'Exponential ↔ log form']}
          nextPointer="Next: the unit circle & trigonometry."
  ...
  return (
    <div className="milo-lesson" style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', background: 'var(--bg-page)', color: 'var(--ink)', fontFamily: 'var(--font-body)' }}>
      <div style={{ width: '100%', maxWidth: 640 }}>
        <TeenTopbar band={BAND} title="Exponential & Log" roundIdx={roundIdx} totalRounds={TOTAL_ROUNDS} onBack={() => { stopSpeech(); onExit() }} />
      </div>
...
// missed concept text-forward, then continues (no penalty, no red).
  ...
  return createPortal(
    <div data-band={BAND} role="dialog" aria-modal="true" style={{ position: 'fixed', inset: 0, zIndex: 950, background: 'color-mix(in srgb, var(--ink) 30%, transparent)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: 'var(--paper)', border: '1px solid var(--outline)', borderRadius: 16, padding: '22px 22px 20px', maxWidth: 520, width: '100%', boxShadow: '0 6px 28px color-mix(in srgb, var(--ink) 18%, transparent)' }}>
        ...
        <ExpLogWatch lines={[round.promptText, round.explain]} onDone={() => setReady(true)} />
...
export default function ExpLogFunctionsChapter(_props: Props) {
  ...
  const finish = useCallback((c: number, w: number, mastered?: boolean) => {
    ...
    finishAndSync('expLogFunctions', c, w, 'practice', mastered)
  }, [finishAndSync])
