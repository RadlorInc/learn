...
export const ITEM_GENERATORS: Record<string, Gen> = {
  ...
  'e.counting10': () => { const n = R(3, 8); return { prompt: `${n - 2}, ${n - 1}, ${n}, ?`, ...num(n + 1) } },
  ...
  'e.matchQty': () => { const n = R(3, 7); return { prompt: `How many? ${glyph().repeat(n)}`, ...num(n) } },
  'e.compare': () => { const a = R(2, 9); let b = R(2, 9); if (b === a) b = a === 9 ? a - 1 : a + 1; return { prompt: `Which is more, ${a} or ${b}? Type it.`, ...num(Math.max(a, b)) } },
  'e.numberOrder': () => { const n = R(3, 8); return { prompt: `What comes just after ${n}?`, ...num(n + 1) } },
  ...
  'e.shapes2d': () => { const [name, n] = pk([['square', 4], ['triangle', 3], ['circle', 0]] as [string, number][]); return { prompt: `How many corners does a ${name} have?`, ...num(n) } },
  ...
  'p.fractionsIntro': () => { const d = pk([2, 3, 4]); return { prompt: `A pizza is cut into ${d} equal parts and you take 1. What fraction is that?`, visual: { t: 'frac', parts: [[1, d]] }, ...frac(1, d) } },
  'p.shapes2d3d': () => { const [name, n] = pk([['triangle', 3], ['square', 4], ['pentagon', 5], ['hexagon', 6]] as [string, number][]); return { prompt: `How many sides does a ${name} have?`, ...num(n) } },
  ...
  'i.measureUnits': () => { const m = R(2, 6); return { prompt: `${m} meters is how many centimeters?`, ...num(m * 100) } },
  ...
  // Stays a PICK: "acute / right / obtuse" is the whole answer space — there is no wider honest one.
  'i.anglesSymmetry': () => { const [d, k] = pk([[40, 'Acute'], [55, 'Acute'], [90, 'Right'], [120, 'Obtuse'], [135, 'Obtuse']] as [number, string][]); return { prompt: `Look at the angle. It measures ${d}°. Is it acute, right, or obtuse?`, visual: { t: 'angle', deg: d }, ...mc(k, (['Acute', 'Right', 'Obtuse'] as string[]).filter(x => x !== k)) } },
  ...
  'i.dataGraphs': () => {
    ...
    const base = shuffle([2, 4, 6, 9]); const vals = base.slice()
    let hi = 0, lo = 0
    for (let i = 1; i < vals.length; i++) { if (vals[i] > vals[hi]) hi = i; if (vals[i] < vals[lo]) lo = i }
    return { prompt: `Look at the chart. How many more ${labels[hi]} than ${labels[lo]}?`, visual: { t: 'bars', labels, vals }, ...num(vals[hi] - vals[lo]) }
  },
  ...
  'm.rationalOps': () => { const b = pk([2, 3, 4]), d = pk([2, 3, 5].filter(x => x !== b)); return { prompt: `1/${b} × 1/${d} = ?`, ...frac(1, b * d) } },
  ...
  // Stays a PICK: a quadrant is one of exactly four names.
  'm.coordinatePlane': () => { const x = pk([-1, 1]) * R(1, 6), y = pk([-1, 1]) * R(1, 6); const q = x > 0 && y > 0 ? 'Quadrant I' : x < 0 && y > 0 ? 'Quadrant II' : x < 0 && y < 0 ? 'Quadrant III' : 'Quadrant IV'; return { prompt: `Look at the grid. Which corner (quadrant) is the point (${x}, ${y}) in?`, visual: { t: 'point', x, y }, ...mc(q, (['Quadrant I', 'Quadrant II', 'Quadrant III', 'Quadrant IV'] as string[]).filter(z => z !== q)) } },
  'm.linearRel': () => { const run = pk([2, 3, 4]), slope = R(2, 5), rise = slope * run; return { prompt: `Look at the line. It goes up ${rise} for every ${run} across. What is its slope?`, visual: { t: 'slope', rise, run }, ...num(slope) } },
  ...
  // Stays a PICK: the answer is an expression, not a number.
  'a.expressions': () => { const a = R(2, 6), b = R(2, 6); return { prompt: `Simplify:  ${a}x + ${b}x`, ...mc(`${a + b}x`, [`${a * b}x`, `${a + b}x²`, `${a + b}`, `${a}x + ${b}`, `${a + b}x³`]) } },
  ...
  'a.systems': () => { const x = R(4, 9), y = R(2, x - 1); return { prompt: `If x + y = ${x + y} and x − y = ${x - y}, what is x?`, ...num(x) } },
  ...
  'a.radicals': () => { const [a, b, c] = pk([[3, 4, 5], [6, 8, 10], [5, 12, 13], [8, 15, 17], [9, 12, 15], [7, 24, 25]] as [number, number, number][]); return { prompt: `Look at the right triangle. Its two legs are ${a} and ${b}. How long is the slanted side (the hypotenuse)?`, visual: { t: 'rtri', a, b, labels: [`${a}`, `${b}`, '?'] }, ...num(c) } },
  // Stays a PICK: the answer is a factored form.
  'a.factoring': () => { const r1 = R(2, 5), r2 = R(2, 6); return { prompt: `Factor:  x² + ${r1 + r2}x + ${r1 * r2}`, ...mc(`(x+${r1})(x+${r2})`, [`(x+${r1})(x+${r2 + 1})`, `(x+${r1 * r2})(x+1)`, `(x+${r1 + r2})(x+1)`, `(x+${r1 + 1})(x+${r2})`, `(x+${r1})(x+${r2 + 2})`]) } },
  'a.quadratics': () => { const r1 = R(2, 5), r2 = r1 + R(1, 4); return { prompt: `Solve:  x² − ${r1 + r2}x + ${r1 * r2} = 0.  Type the SMALLER solution.`, ...num(r1) } },
  ...
  'c.polynomialFns': () => { const d = R(3, 6), a = R(2, 5), b = R(2, 6), c = R(1, 9); return { prompt: `What is the degree of  ${a}x^${d} + ${b}x² + ${c} ?`, ...num(d) } },
  ...
  'c.expLog': () => { const b = pk([2, 3, 5]), e = R(2, 4); return { prompt: `log_${b}(${Math.pow(b, e)}) = ?`, ...num(e) } },
  ...
  'c.trigGraphsId': () => { const a = R(2, 6); return { prompt: `What is the amplitude of  y = ${a} sin(x) ?`, ...num(a) } },
  ...
  'c.statsInference': () => { const m = R(3, 8), nums = shuffle([m - 3, m - 1, m + 1, m + 3]); return { prompt: `What is the mean (average) of  ${nums.join(', ')} ?`, ...num(m) } },
  'c.introCalculus': () => { const n = R(3, 6); return { prompt: `The derivative of x^${n} is ${n}x^k.  What is k?`, ...num(n - 1) } },
}
