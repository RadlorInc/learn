 ...
 * easy = counting / recognition / colors / shapes; medium adds comparison;
...
const COLORS = [{ n: 'red', e: '🔴' }, { n: 'blue', e: '🔵' }, { n: 'green', e: '🟢' }, { n: 'yellow', e: '🟡' }, { n: 'purple', e: '🟣' }]
...
function color(): Quiz {
  const c = COLORS[rint(0, COLORS.length - 1)]
  const claim = coin() ? c : COLORS[rint(0, COLORS.length - 1)]
...
export function makeQuiz(difficulty: 1 | 2 | 3): Quiz {
  const easy = [() => counting(5), () => recognition(5), color, shape]
