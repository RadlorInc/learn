 ...
 *
 * The chapter the learner last played is cached per-device (IndexedDB via kv) with
...
export function getLastPlayed(learnerId: string): LastPlayed | null {
  try {
    const raw = kv.get(key(learnerId))
    if (!raw) return null
...
export function setLastPlayed(learnerId: string, chapter: ChapterType, at: number = Date.now()) {
  try {
    kv.set(key(learnerId), JSON.stringify({ chapter, at }))
  } catch {}
}
